(function(){
  ...
  function makeProject(templateId="salon-spa"){
    ...
    return {id:uid("project"),name:spec.name,template:spec.id,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),website:{name:spec.name,pages:[home,about,contact],globalStyles:{primary:spec.accent,secondary:spec.secondary,accent:"#ffb020",background:spec.bg,text:spec.text,muted:spec.muted,headingFont:"Manrope",bodyFont:"Inter",buttonFont:"Inter",buttonRadius:999,cardRadius:26,cardBorder:"rgba(15,23,42,.10)",cardShadow:"0 20px 50px rgba(15,23,42,.10)"},integrations:{orders:"",booking:"",menu:"",refill:"",businessCard:""},settings:{businessName:spec.name,logo:"",favicon:"",phone:"+91 90000 00000",email:"hello@example.com",address:"Your business address",whatsapp:"919000000000",instagram:"",facebook:"",youtube:"",maps:"",businessCard:"",stickyHeader:true,transparentHeader:false,navCtaText:primaryCta,navCtaLink:spec.kind==="service"?"g58-booking":spec.kind==="menu"?"g58-menu":"g58-orders"},assets:[],products:spec.kind==="product"?clone(cards.elements):[],services:spec.kind==="service"?clone(cards.elements):[],menu:spec.kind==="menu"?clone(cards.elements):[]}};
  }
  ...
  function resolveLink(link,project){
    ...
    const raw=l.useGlobal?({"g58-orders":integrations.orders,"g58-booking":integrations.booking,"g58-menu":integrations.menu,"g58-refill":integrations.refill,"g58-card":integrations.businessCard,whatsapp:settings.whatsapp,instagram:settings.instagram,facebook:settings.facebook,youtube:settings.youtube,maps:settings.maps,phone:settings.phone,email:settings.email}[l.type]||l.value):l.value;
    if(!raw)return "#";
  ...
  function floatingMarkup(project,editor=false){
    ...
    return `<div class="wb-floating"${editor?' data-editor-region="floating" tabindex="0"':""}><a class="wa" href="${escapeHtml(resolveLink({type:"whatsapp",value:"",useGlobal:true},project))}" aria-label="WhatsApp">${whatsappIcon}</a><a class="ig" href="${escapeHtml(resolveLink({type:"instagram",value:"",useGlobal:true},project))}" aria-label="Instagram">${instagramIcon}</a><a class="g58" href="${escapeHtml(integrations.businessCard||s.businessCard||"#")}" aria-label="G58 Business Card"><svg viewBox="0 0 120 120" fill="none" stroke="currentColor" stroke-width="8"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg></a></div>`;
  }
