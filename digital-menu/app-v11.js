...
function activeMenuEntitlement(){
  if(CONFIG.testMode===true)return {plan:'paid',ordersEnabled:true,maxRestaurants:5,permanentOrders:false,messaging:true,reports:true,scheduling:true,customerSubscriptions:true,posPremium:true,lifetime:true};
  ...
  return {...entitlement,plan:'paid',ordersEnabled:true,maxRestaurants:5,permanentOrders:false,messaging:true,reports:true,scheduling:true,customerSubscriptions:true,posPremium:true};
}
function menuFeature(name){return !!activeMenuEntitlement()[name]}
function menuPlanLabel(){return activeMenuEntitlement().plan==='paid'?'Digital Menu Monthly':'Free Digital Menu'}
...
function pricingRows(){
  return menuPricing.periods.map(period=>({...period,amount:PLAN_UTILS.priceFor(menuPricing.monthly,period),link:menuPricing.links?.[`paid_${period.id}`]||''}));
}
...
function ownerPeriodControls(label='Filter dashboard'){
  ...
  return `<div class="owner-period-filter" aria-label="${html(label)}"><label><span>View by</span><select id="ownerPeriodMode" disabled><option value="day" selected>Today</option></select></label><label><span>${modeLabel}</span><input value="${html(period.value)}" disabled aria-label="Today only"></label><small>Orders reset daily. Export today’s CSV before reset.</small></div>`;
}
...
async function syncDigitalMenuAccount(){
  ...
  for(const restaurant of ownerRestaurants()){
    ...
    restaurant.premiumFeatures=features.plan==='paid';
...
function registerForm(){return `<form id="registerForm"><div class="form-grid"><div class="field"><label>Full name</label><input name="name" required></div><div class="field"><label>Email</label><input name="email" type="email" required></div><div class="field"><label>Mobile</label><input name="mobile"></div><div class="field"><label>City</label><input name="city"></div><div class="field"><label>Password</label><input name="password" type="password" required></div><div class="field"><label>Confirm password</label><input name="confirm" type="password" required></div><label class="field full"><span><input name="retentionAccepted" type="checkbox" required> I accept the <a href="/terms/" target="_blank" rel="noopener">Terms</a> and understand that Digital Menu order data resets every 24 hours. I can export today’s orders as CSV before reset.</span></label></div><button class="btn full">Create Account</button></form>`}
...
function renderOwnerOnboarding(){app.innerHTML=`<main class="screen auth"><section class="auth-card glass"><a class="menu-home-link" href="../">← Gravity58 Home</a><div class="premium-menu-kicker">SET UP YOUR RESTAURANT</div><a class="brand" href="../"><svg class="brand-mark" viewBox="0 0 120 120" fill="none" stroke="#c96a2a" stroke-width="8" aria-hidden="true"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg><div><h2>Create your first Digital Menu</h2><p class="tagline">Your restaurant and menu are securely saved to your G58 account.</p></div></a><button class="btn full" id="createFirstRestaurant">Add Restaurant</button><button class="link-btn full" id="onboardingLogout">Logout</button></section></main>`;$('#createFirstRestaurant').onclick=()=>openRestaurantForm(true);$('#onboardingLogout').onclick=logoutOwner}

function renderShell(){const r=activeRestaurant();if(!r)return renderOwnerOnboarding();app.innerHTML=`<div class="shell"><aside class="sidebar"><a class="brand" href="../"><svg class="brand-mark" viewBox="0 0 120 120" fill="none" stroke="#c96a2a" stroke-width="8" aria-hidden="true"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg><div><strong>Gravity58 Menu</strong><small class="muted">Restaurant workspace</small></div></a><nav class="nav">${navButton('dashboard','⌂','Dashboard')}${navButton('restaurants','◫','Restaurants')}${navButton('menu','☰','Menu')}${navButton('orders','◉','Orders')}${navButton('schedule','◷','Schedule')}${navButton('subscriptions','♢','Subscriptions')}${navButton('qr','▦','QR Codes')}${navButton('reports','◒','Reports')}${navButton('pricing','₹','Pricing')}${navButton('publish','⇧','Share Menu')}${navButton('settings','⚙','Settings')}<a class="owner-book-ad" href="${CONFIG.adBookingPortalUrl||'../advertise/'}?restaurant=${encodeURIComponent(`${r.name}|${r.city}`)}">✦ Book Ad Space</a><button id="logout">⇥ Logout</button></nav></aside><main class="main"><header class="topbar"><div class="restaurant-switch"><span>${r.name?.[0]||'G'}</span><select id="restaurantSelect">${ownerRestaurants().map(x=>`<option value="${x.id}" ${x.id===r.id?'selected':''}>${x.name}</option>`).join('')}</select><button class="btn small secondary" id="addRestaurant">+ Add</button></div><nav class="g58-portal-icon-nav" aria-label="Quick links"><a href="/" aria-label="Home"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 11.5 12 4l9 7.5"/><path d="M5.5 10v9a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-9"/><path d="M9.5 20v-6h5v6"/></svg><span>Home</span></a><a href="/digit58/" aria-label="Refills"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 11A8 8 0 0 0 6.3 6.3L4 8.6"/><path d="M4 4v4.6h4.6"/><path d="M4 13a8 8 0 0 0 13.7 4.7L20 15.4"/><path d="M20 20v-4.6h-4.6"/></svg><span>Refills</span></a><a href="/digital-menu/" aria-label="Digital Menu"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7 3v6a2 2 0 0 0 2 2h0"/><path d="M9 3v18"/><path d="M7 3v5"/><path d="M15 3c-1.2 1.4-1.2 6.6 0 8 .5.6 1 .8 1 1.5V21"/></svg><span>Digital Menu</span></a></nav><div class="user-label"><span class="plan-badge plan-${activeMenuEntitlement().plan}">${menuPlanLabel()}</span><button class="status-pill sync-menu-button" id="syncCloudMenu"><span class="dot"></span> Menu synced</button></div></header><section class="content" id="page"></section></main></div><footer class="g58-site-footer"><div class="g58-site-footer-badge"><a class="g58-app-badge" href="/downloads/GRAVITY58-Android-v1.9.apk" download aria-label="Download the Gravity58 Android app"><span class="g58-app-badge-icon">▶</span><span class="g58-app-badge-text"><small>Download Android</small><strong>Get G58 App</strong></span></a></div><p class="g58-site-footer-note">© ${new Date().getFullYear()} Gravity58 · Digital Menu</p></footer>${activeMenuEntitlement().plan==='paid'?'<a class="floating-support-btn" href="/support/?from=digital-menu" title="Support">🛟<span>Support</span></a>':''}`;
...
function dashboardView(){const r=activeRestaurant(),items=restaurantItems(),orders=ordersForOwnerPeriod(restaurantOrders()),period=ownerPeriodLabel(),completed=orders.filter(x=>x.status==='Completed'),posSync=menuFeature('posPremium')&&isCloudMenuSession()?`<article class="card premium-pos-sync-card"><div><span class="eyebrow">RESTAURANT POS</span><strong class="premium-pos-sync-title">Restaurant billing workspace</strong><p class="muted">Open the complete POS workspace for ${html(r.name)} with its order data selected.</p></div><a class="btn premium-pos-button" href="${premiumPosUrl(r)}">▣ Open Restaurant POS</a></article>`:'';$('#page').innerHTML=`<div class="hero"><div><span class="status-pill"><span class="dot"></span>${r.open?'Open':'Closed'}</span><h1 style="margin:12px 0 6px">${r.logo} ${r.name}</h1><p>${r.description||r.type+' in '+r.city}</p></div><div class="actions"><button class="btn secondary" id="previewMenu">Preview Menu</button><button class="btn secondary" id="openOrders">Open Orders</button><button class="btn" id="openCsv">Manage Menu</button></div></div>${posSync}<div class="dashboard-filter-row"><div><h2>${html(period)} overview</h2><p class="muted">Today’s order totals. Digital Menu order data resets every 24 hours.</p></div>${ownerPeriodControls('Today’s restaurant dashboard')}</div><div class="grid stats">${metric('Categories',restaurantCategories().length)}${metric('Menu Items',items.length)}${metric('Orders',orders.length)}${metric('Revenue',money(completed.reduce((a,b)=>a+b.total,0)))}</div><div class="section-head"><div><h2>Orders for ${html(period)}</h2><p class="muted">${orders.length} order${orders.length===1?'':'s'} today</p></div><button class="btn secondary" id="exportDashboardOrders">Export Today CSV</button></div>${orderCompactTable(orders.slice(0,12),{actions:false,emptyText:'No orders today'})}`;
  bindOwnerPeriodControls(dashboardView);$('#exportDashboardOrders').onclick=()=>exportDigitalMenuOrders(orders,r);$('#previewMenu').onclick=()=>{location.href=cloudCustomerMenuUrl(r)};$('#openCsv').onclick=()=>{view='menu';renderShell()};$('#openOrders').onclick=()=>{view=menuFeature('ordersEnabled')?'orders':'pricing';renderShell();if(view==='pricing')toast('Request Digital Menu activation to receive orders')};}
...
function pricingView(){
  ...
  const planCards=pricingRows().map(period=>`<article class="card pricing-card paid"><span>MONTHLY</span><h3>${period.label}</h3><div class="pricing-amount">${money(period.amount)}<small> / month</small></div><small>Up to 5 restaurants, POS sync, reports, messaging, subscriptions and scheduled orders. Orders reset every 24 hours; export today’s CSV before reset.</small><button class="btn full" data-request-plan="paid" data-period="${period.id}">${period.link?'Pay & Request Activation':'Request Activation'}</button></article>`).join('');
  $('#page').innerHTML=`<div class="section-head"><div><h1>Digital Menu Pricing</h1><p class="muted">Your first restaurant and live orders are free. Unlock the complete restaurant suite for ₹699 per month.</p></div><span class="plan-badge plan-${entitlement.plan}">${menuPlanLabel()}</span></div><article class="card entitlement-summary"><div><strong>Current access</strong><h2>${menuPlanLabel()}</h2><p>${restaurantLimit()} restaurant slot(s) · ${entitlement.ordersEnabled?'Orders enabled':'Menu-only access'} · ${expiry}</p></div>${pending.length?`<span class="chip">${pending.length} request(s) awaiting G58</span>`:''}</article><section class="pricing-plan-section"><div class="section-head"><div><span class="eyebrow">COMPLETE DIGITAL MENU</span><h2>Monthly Subscription</h2><p class="muted">One simple plan with every paid Digital Menu feature included.</p></div><strong class="pricing-from">${money(menuPricing.monthly)}/month</strong></div><div class="pricing-grid">${planCards}</div></section>`;
...
async function requestMenuActivation(plan,periodId){
  ...
  const period=pricingRows().find(row=>row.id===periodId);if(!period||plan!=='paid')return;
  ...
  modal(id?'Edit Restaurant':first?'Create Your First Restaurant':'Add Restaurant',restaurantForm(r),()=>{
    ...
    form.onsubmit=async e=>{
      ...
      try{
        ...
        else{const features=activeMenuEntitlement();saved={id:uid('res'),ownerId:state.session.userId,...d,logo:'G',restaurantKey:`${d.name}|${d.city}`,tax:+d.tax,service:0,open:true,accepting:true,social:{},subscriptionPlans:[],digitalMenuPlan:features.plan,ordersEnabled:features.ordersEnabled,premiumFeatures:features.plan==='paid',entitlementExpiresAt:features.lifetime?'':features.expiresAt||''};state.restaurants.push(saved);state.activeRestaurantId=saved.id}
...
function orderCsvCell(value){const text=String(value??'');return /[",\n]/.test(text)?`"${text.replace(/"/g,'""')}"`:text}
function exportDigitalMenuOrders(orders=restaurantOrders(),restaurant=activeRestaurant()){
  const rows=[['Token','Order ID','Restaurant','Customer','Phone','Service','Table','Items','Subtotal','Tax','Service charge','Total','Status','Scheduled for','Created at']];
  orders.forEach(order=>rows.push([formatToken(order.tokenNumber),order.id||order.$id,restaurant?.name||'',order.customerName||order.customer||'Guest',order.phone||'',order.serviceMode||'',order.tableNumber||'',(order.items||[]).map(item=>`${Number(item.qty)||1} x ${item.name}`).join(' | '),order.subtotal||'',order.tax||'',order.serviceCharge||'',order.total||0,order.status||'',order.scheduledFor||'',order.createdAt||order.$createdAt||'']));
  const csv=rows.map(row=>row.map(orderCsvCell).join(',')).join('\n');
  downloadFile(`g58-digital-menu-orders-${indiaDateValue()}-${slugify(restaurant?.name||'restaurant')}.csv`,csv,'text/csv;charset=utf-8');
  toast(`${Math.max(0,rows.length-1)} order(s) exported`);
}
...
function ordersView(){
  ...
  $('#page').innerHTML=`<div class="section-head"><div><h1>Live Orders <small class="order-reset-copy">(orders reset in <span id="orderResetCountdown">${orderResetCountdown()}</span>)</small></h1><p class="muted">Instant order board · ${active.length} active token(s) pinned · export today’s orders before the daily reset.</p></div><div class="orders-header-actions"><button class="btn small secondary" id="exportTodayOrders">Export Today CSV</button>${posButton}<span class="status-pill live-sync-pill"><span class="dot"></span> Live sync & sound alerts</span></div></div><div class="order-controls"><div class="tabs">${statuses.map(s=>`<button class="btn small ${s===ownerOrderStatusFilter?'':'secondary'}" data-tab="${s}">${s}</button>`).join('')}</div></div><div id="ordersGrid" class="orders-board">${ordersBoardMarkup(visible,`No ${ownerOrderStatusFilter==='All'?'':ownerOrderStatusFilter.toLowerCase()+' '}orders today`)}</div>`;
  clearInterval(orderResetTimer);orderResetTimer=setInterval(()=>{const target=$('#orderResetCountdown');if(target)target.textContent=orderResetCountdown()},1000);
  $('#exportTodayOrders').onclick=()=>exportDigitalMenuOrders(periodOrders,activeRestaurant());
...
function orderMessagesMarkup(o,role='owner'){
  if(!menuFeature('messaging'))return '<div class="premium-inline-lock">Customer messaging is available with the Digital Menu monthly subscription.</div>';
...
function reportsView(){if(!menuFeature('reports')){view='pricing';return pricingView()}const orders=ordersForOwnerPeriod(restaurantOrders()),sales=orders.filter(x=>x.status==='Completed').reduce((a,b)=>a+b.total,0),period=ownerPeriodLabel();$('#page').innerHTML=`<div class="section-head"><div><h1>Today’s Reports</h1><p class="muted">Digital Menu order data resets every 24 hours. Export today’s CSV before the reset.</p></div><button class="btn secondary" id="exportReportOrders">Export Today CSV</button></div><div class="grid stats">${metric('Total Orders',orders.length)}${metric('Completed',orders.filter(x=>x.status==='Completed').length)}${metric('Total Sales',money(sales))}${metric('Average Order',money(orders.length?orders.reduce((a,b)=>a+b.total,0)/orders.length:0))}</div><div class="section-head"><h2>All Restaurants Overview · ${html(period)}</h2></div><div class="grid restaurant-grid">${ownerRestaurants().map(r=>{const os=ordersForOwnerPeriod(restaurantOrders(r.id));return `<article class="card"><h3>${r.logo} ${r.name}</h3><p class="muted">${os.length} orders · ${money(os.filter(x=>x.status==='Completed').reduce((a,b)=>a+b.total,0))} sales</p></article>`}).join('')}</div>${orderCompactTable(orders,{actions:false,emptyText:'No report data today'})}`;$('#exportReportOrders').onclick=()=>exportDigitalMenuOrders(orders,activeRestaurant());bindOwnerPeriodControls(reportsView)}
...
function scheduleView(){
  ...
  $('#page').innerHTML=`<div class="section-head"><div><h1>Scheduled Orders</h1><p class="muted">Orders appear here once they're within 15 minutes of their scheduled time, and ring continuously — like an incoming call — until started.</p></div><span class="status-pill"><span class="dot"></span> Subscription scheduling</span></div><div class="grid order-grid">${scheduled.map(order=>{const ringing=ringingOrderIds.has(order.id);return `<article class="card schedule-card ${ringing?'incoming-order':''}">${ringing?'<span class="incoming-order-beacon" aria-label="Scheduled order starting" title="Scheduled order starting"></span>':''}<span class="eyebrow">TOKEN ${formatToken(order.tokenNumber)}</span><h2>${new Date(order.scheduledFor).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'})}</h2><p><strong>${html(order.customerName||order.customer)}</strong> · ${order.items.map(item=>`${item.qty} × ${html(item.name)}`).join(', ')}</p><div class="chips"><span class="chip">${html(order.status)}</span><span class="chip">${money(order.total)}</span></div><div class="actions">${orderActions(order)}<button class="btn small secondary" data-print-order="${html(order.id)}">Print</button></div></article>`}).join('')||empty('No scheduled orders')}</div>`;
...
function subscriptionsView(){
  ...
  $('#page').innerHTML='<div class="remote-menu-loading"><svg class="brand-spinner" viewBox="0 0 120 120" fill="none" stroke="#ec6e20" stroke-width="9" aria-hidden="true"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg><h2>Loading subscriptions</h2></div>';
...
function drawSubscriptionsView(){
  ...
  $$('[data-pause-sub]').forEach(button=>button.onclick=()=>pauseSubscription(button.dataset.pauseSub));
  $$('[data-resume-sub]').forEach(button=>button.onclick=()=>pauseSubscription(button.dataset.resumeSub));
  $$('[data-extend-sub]').forEach(button=>button.onclick=()=>extendSubscription30Days(button.dataset.extendSub));
  $$('[data-stop-sub]').forEach(button=>button.onclick=()=>stopSubscription(button.dataset.stopSub));
  $$('[data-remove-sub]').forEach(button=>button.onclick=()=>removeSubscription(button.dataset.removeSub));
...
async function pauseSubscription(id){
  const row=mealSubscriptions.find(item=>(item.id||item.$id)===id);if(!row)return;
  const changes={status:row.status==='Paused'?'Active':'Paused',updatedAt:now()};
  try{if(isCloudMenuSession())Object.assign(row,await Gravity58Ads.update(ownerSubscriptionKind(),id,changes));else Object.assign(row,changes);drawSubscriptionsView();toast(changes.status==='Paused'?'Subscription paused':'Subscription resumed')}
  catch(error){toast(error.message||'Could not update subscription')}
}
async function extendSubscription30Days(id){
  const row=mealSubscriptions.find(item=>(item.id||item.$id)===id);if(!row)return;
  const base=row.nextScheduledMeal&&new Date(row.nextScheduledMeal).getTime()>Date.now()?new Date(row.nextScheduledMeal).getTime():Date.now();
  const changes={nextScheduledMeal:new Date(base+30*86400000).toISOString(),updatedAt:now()};
  try{if(isCloudMenuSession())Object.assign(row,await Gravity58Ads.update(ownerSubscriptionKind(),id,changes));else Object.assign(row,changes);drawSubscriptionsView();toast('Subscription extended by 30 days')}
  catch(error){toast(error.message||'Could not extend subscription')}
}
async function stopSubscription(id){
  const row=mealSubscriptions.find(item=>(item.id||item.$id)===id);if(!row||!confirm('Stop this subscription? The customer will see it as cancelled.'))return;
  const changes={status:'Cancelled',updatedAt:now()};
  try{if(isCloudMenuSession())Object.assign(row,await Gravity58Ads.update(ownerSubscriptionKind(),id,changes));else Object.assign(row,changes);drawSubscriptionsView();toast('Subscription stopped')}
  catch(error){toast(error.message||'Could not stop subscription')}
}
async function removeSubscription(id){
  const row=mealSubscriptions.find(item=>(item.id||item.$id)===id);if(!row||!confirm('Permanently remove this subscription account? This cannot be undone.'))return;
  try{if(isCloudMenuSession())await Gravity58Ads.remove(ownerSubscriptionKind(),id);mealSubscriptions=mealSubscriptions.filter(item=>(item.id||item.$id)!==id);drawSubscriptionsView();toast('Subscription removed')}
  catch(error){toast(error.message||'Could not remove subscription')}
}
...
async function loadRemoteMenuConfig(source){
  if(remoteMenuLoading)return;remoteMenuLoading=true;app.innerHTML='<main class="public-menu"><div class="remote-menu-loading"><svg class="brand-spinner" viewBox="0 0 120 120" fill="none" stroke="#ec6e20" stroke-width="9" aria-hidden="true"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg><h2>Loading published menu</h2><p>Downloading the restaurant config securely…</p></div></main>';
...
async function loadCloudMenuConfig(recordId,ownerId){
  ...
  app.innerHTML='<main class="public-menu"><div class="remote-menu-loading"><svg class="brand-spinner" viewBox="0 0 120 120" fill="none" stroke="#ec6e20" stroke-width="9" aria-hidden="true"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg><h2>Loading restaurant menu</h2><p>Reading the latest menu from Gravity58…</p></div></main>';
  try{
...
async function renderCustomerSubscriptionRoute(){
  ...
  try{
    if(remoteMenuSource!==`cloud:${ownerId}:${recordId}`){app.innerHTML='<main class="subscription-dashboard-page"><div class="remote-menu-loading"><svg class="brand-spinner" viewBox="0 0 120 120" fill="none" stroke="#ec6e20" stroke-width="9" aria-hidden="true"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg><h2>Loading customer dashboard</h2></div></main>';const record=await Gravity58Ads.get(cloudMenuKind(ownerId),recordId);if(record.ownerId!==ownerId)throw new Error('Restaurant owner does not match this dashboard link');remoteMenuConfig=cloudRecordToConfig(record);remoteMenuSource=`cloud:${ownerId}:${recordId}`;cacheCloudMenuForOrdersSafely(remoteMenuConfig,ownerId)}
...
async function openCart(r,{scheduleAfterAuth=false}={}){
  ...
  modal('Your Cart',`<div id="checkoutPanel">
    ...
    button.addEventListener('click',async()=>{
      ...
      if(scheduledValue&&!scheduleAccountReady)return fail('Login with your scheduled-meal account before scheduling an order.');
