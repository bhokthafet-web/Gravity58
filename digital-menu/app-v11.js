...
function orderActions(o){const map={'Payment Verification':['Confirm Payment','Reject Payment'],Pending:['Accept','Reject'],Accepted:['Done'],Preparing:['Done'],Ready:['Delivered'],Delivered:['Complete']};return (map[o.status]||[]).map(a=>`<button class="btn small ${a==='Reject'?'red':a==='Done'?'green':''}" data-order="${o.id}" data-action="${a}">${a}</button>`).join('')}
function bindOrderActions(){$$('[data-order]').forEach(b=>b.onclick=()=>updateOrder(b.dataset.order,b.dataset.action))}
function updateOrder(id,action){const o=state.orders.find(x=>x.id===id);const next={Accept:'Accepted',Reject:'Rejected','Confirm Payment':'Pending','Reject Payment':'Payment Rejected',Preparing:'Preparing',Done:'Ready',Delivered:'Delivered',Complete:'Completed'}[action];if(!next)return;o.status=next;if(action==='Confirm Payment')o.paymentStatus='Confirmed';if(action==='Reject Payment')o.paymentStatus='Rejected';o.updatedAt=now();save();toast(action==='Done'?`Order ${o.id}: Food is ready`:`Order ${o.id}: ${next}`);view==='orders'?ordersView():dashboardView()}
...
function qrView(){const r=activeRestaurant(),url=`${location.href.split('#')[0]}#menu&restaurant=${r.id}`;$('#page').innerHTML=`<div class="section-head"><div><h1>QR Codes</h1><p class="muted">General restaurant menu QR code</p></div><button class="btn secondary" id="previewQr">Preview Menu</button></div><article class="card qr-card"><h2>${r.logo} ${r.name}</h2><p>Scan to View Menu</p><div class="qr-wrap"><div id="qrcode"></div></div><p class="muted" style="word-break:break-all;margin-top:14px">${url}</p><div class="actions"><button class="btn" id="copyQr">Copy Menu Link</button><button class="btn secondary" onclick="window.print()">Print</button></div><small class="muted" style="margin-top:16px">Powered by Gravity58 Digital Menu</small></article>`;try{new QRCode($('#qrcode'),{text:url,width:220,height:220})}catch{$('#qrcode').innerHTML='<div style="color:#111;padding:70px 25px">QR library requires internet once</div>'}$('#copyQr').onclick=()=>navigator.clipboard?.writeText(url).then(()=>toast('Menu link copied'));$('#previewQr').onclick=()=>location.hash=`menu&restaurant=${r.id}`}
...
function modal(title,body,onReady){document.body.insertAdjacentHTML('beforeend',`<div class="modal-backdrop" id="modal"><section class="modal"><div class="section-head" style="margin-top:0"><h2>${title}</h2><button class="btn small secondary" id="closeModal">✕</button></div>${body}</section></div>`);$('#closeModal').onclick=closeModal;onReady?.()}
