...
function orderActions(o){const map={'Payment Verification':['Confirm Payment','Reject Payment'],Pending:['Accept','Reject'],Accepted:['Done'],Preparing:['Done'],Ready:['Complete']};return (map[o.status]||[]).map(a=>`<button class="btn small ${a==='Reject'?'red':a==='Done'||a==='Complete'?'green':''}" data-order="${o.id}" data-action="${a}">${a}</button>`).join('')}
