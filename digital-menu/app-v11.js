...
function localMediaSource(record,keyName,legacyName){return localMediaUrls.get(record?.[keyName])||record?.[legacyName]||''}
...
function renderLogin(){app.innerHTML=`<main class="screen auth"><section class="auth-card glass"><a class="menu-home-link" href="../">← Gravity58 Home</a><div class="premium-menu-kicker">PREMIUM RESTAURANT EXPERIENCE</div><div class="brand"><div class="brand-mark">G</div><div><h2>Gravity58 Digital Menu</h2><p class="tagline">Scan. Order. Relax.</p></div></div><form id="loginForm"><div class="field"><label>User ID or Email</label><input name="email" value="demo@g58.in" required></div><div class="field"><label>Password</label><input name="password" type="password" value="demo123" required></div><button class="btn full">Login</button></form><div class="actions" style="justify-content:center;margin-top:12px"><button class="link-btn" id="newUser">New User</button><button class="link-btn" id="forgot">Forgot Password</button></div><p class="muted" style="text-align:center;margin-top:18px">Restaurant accounts and operational data remain in this browser.</p></section></main>`;
  $('#loginForm').onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target)),button=e.submitter;button.disabled=true;button.textContent='Signing in…';const local=state.users.find(x=>(x.email===d.email||x.id===d.email)&&x.password===d.password);if(local){state.session={userId:local.id};save();return render()}try{if(!Gravity58Ads?.configured)throw new Error('Invalid login');await Gravity58Ads.login(d.email.trim(),d.password);const account=await Gravity58Ads.currentUser();if(!account)throw new Error('Could not read this account');const user=ensureGravity58User(account);state.session={userId:user.id,provider:'gravity58'};save();render()}catch(error){button.disabled=false;button.textContent='Login';toast(error.message||'Invalid login')}};
  $('#newUser').onclick=()=>modal('Create Account',registerForm(),bindRegister);
  $('#forgot').onclick=()=>modal('Password Recovery',`<p class="muted">For a Gravity58 account, send a secure reset email. Local-only demo accounts can be reset directly on this browser.</p><form id="forgotForm"><div class="field"><label>Email</label><input name="email" type="email" required></div><div class="field"><label>New local password <small>(local account only)</small></label><input name="password" type="password" minlength="6"></div><div class="field"><label>Confirm local password</label><input name="confirm" type="password" minlength="6"></div><div class="actions"><button class="btn" name="action" value="email">Send G58 Reset Email</button><button class="btn secondary" name="action" value="local">Reset Password</button></div></form>`,()=>{$('#forgotForm').onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target)),action=e.submitter?.value||'email';if(action==='email'){try{await Gravity58Ads.forgotPassword(d.email.trim(),location.origin+'/reset-password/');closeModal();return toast('Password reset email sent')}catch(error){return toast(error.message||'Could not send reset email')}}const u=state.users.find(x=>x.email.toLowerCase()===d.email.trim().toLowerCase());if(!u)return toast('No local account exists for this email');if((d.password||'').length<6)return toast('Use at least 6 characters');if(d.password!==d.confirm)return toast('Passwords do not match');u.password=d.password;save();closeModal();toast('Password reset on this browser')}});
}
  ...
  modal(id?'Edit Restaurant':first?'Create Your First Restaurant':'Add Restaurant',restaurantForm(r),()=>{
    ...
    form.onsubmit=async e=>{
      ...
      try{
        ...
        if(selectedFile?.size){const blob=file._optimizedBlob||await optimizeLocalImage(selectedFile,{maxWidth:720}),key=saved.logoImageKey||`restaurant:${saved.id}`;await putLocalMedia(key,blob);saved.logoImageKey=key;saved.logoImage=''}
        save();Gravity58Ads?.upsertSlot({id:saved.id,restaurantKey:saved.restaurantKey,name:saved.name,city:saved.city,active:true}).catch(()=>{});if(previewUrl)URL.revokeObjectURL(previewUrl);closeModal();renderShell();toast('Restaurant saved in this browser')
      }catch(error){button.disabled=false;button.textContent='Save Restaurant';toast(error.message||'Could not save restaurant')}
    };
  });
}
async function deleteRestaurant(id){const restaurant=state.restaurants.find(r=>r.id===id);if(!restaurant||!confirm(`Permanently delete ${restaurant.name} and its local menu, orders and QR locations?`))return;const imageKeys=[restaurant.logoImageKey,...state.items.filter(row=>row.restaurantId===id).map(row=>row.imageKey)].filter(Boolean);state.restaurants=state.restaurants.filter(r=>r.id!==id);state.categories=state.categories.filter(row=>row.restaurantId!==id);state.items=state.items.filter(row=>row.restaurantId!==id);state.orders=state.orders.filter(row=>row.restaurantId!==id);state.locations=state.locations.filter(row=>row.restaurantId!==id);const next=ownerRestaurants()[0];state.activeRestaurantId=next?.id||'';save();Promise.all(imageKeys.map(deleteLocalMedia)).catch(()=>{});Gravity58Ads?.upsertSlot({id:restaurant.id,restaurantKey:restaurant.restaurantKey||`${restaurant.name}|${restaurant.city}`,name:restaurant.name,city:restaurant.city,active:false}).catch(()=>{});toast('Restaurant deleted');next?restaurantsView():renderOwnerOnboarding()}
function menuView(){const cats=restaurantCategories(),items=restaurantItems();$('#page').innerHTML=`<div class="section-head"><div><h1>Menu Setup</h1><p class="muted">Add an item, type any category name and upload its food image.</p></div><button class="btn" id="addItem">+ Menu Item</button></div><div class="toolbar"><input id="itemSearch" placeholder="Search menu items"><select id="catFilter"><option value="">All categories</option>${cats.map(c=>`<option value="${c.id}">${html(c.name)}</option>`).join('')}</select></div><div class="grid menu-grid" id="menuGrid">${items.map(menuCard).join('')||empty('Create your first menu item')}</div>`;$('#addItem').onclick=()=>openItemForm();$('#itemSearch').oninput=filterItems;$('#catFilter').onchange=filterItems;bindMenuActions()}
...
async function importMenuConfigFile(file){
  ...
  try{
    ...
    state.restaurants.push(restaurant);state.categories.push(...categories);state.items.push(...items);state.activeRestaurantId=rid;save();Gravity58Ads?.upsertSlot({id:rid,restaurantKey:restaurant.restaurantKey,name:restaurant.name,city:restaurant.city,active:true}).catch(()=>{});closeModal();view='publish';renderShell();toast('Menu config imported into this browser')
...
function publishSetupView(){
  const r=activeRestaurant();$('#page').innerHTML=`<div class="section-head"><div><h1>Publish & Setup</h1><p class="muted">Turn this browser-local restaurant into a customer-visible static menu.</p></div></div><section class="publish-setup-grid"><article class="card publish-primary"><span class="publish-step">01</span><h2>Download your menu config</h2><p>The JSON file contains this restaurant, categories, menu items and compressed food images. It contains no owner password and does not write menu data to Appwrite.</p><div class="actions"><button class="btn" id="downloadMenuConfig">Download Menu Config</button><button class="btn secondary" id="importMenuConfig">Import Config</button><input id="importConfigFile" type="file" accept="application/json,.json" hidden></div></article><article class="card publish-instructions"><span class="publish-step">02</span><h2>Host the config file</h2><ol><li>Upload the downloaded JSON file to your own public HTTPS hosting, such as GitHub Pages, Netlify or your website.</li><li>The host must allow public browser downloads. GitHub Pages works; on other hosts enable <code>Access-Control-Allow-Origin: *</code> if required.</li><li>Copy the direct public URL ending in <code>.json</code>, paste it below, and generate the customer menu link and QR.</li><li>Whenever the menu changes, download the config again and replace the hosted JSON file using the same filename.</li></ol><div class="publish-note"><strong>Important:</strong> browser-local orders cannot travel back to the owner’s device. This static publication is a customer-viewable menu. Cross-device ordering requires a shared orders service.</div></article></section><article class="card publish-link-builder"><span class="publish-step">03</span><h2>Create the customer link</h2><form id="publishUrlForm"><div class="field"><label>Public config file URL</label><input name="configUrl" type="url" value="${html(r.publishedConfigUrl||'')}" placeholder="https://your-site.com/menu-config.json" required></div><button class="btn">Save URL & Generate Link</button></form><div id="publishResult"></div></article>`;
...
function qrView(){const r=activeRestaurant(),published=!!r.publishedConfigUrl,url=published?customerMenuUrl(r.publishedConfigUrl):`${location.href.split('#')[0]}#menu&restaurant=${r.id}`;$('#page').innerHTML=`<div class="section-head"><div><h1>QR Codes</h1><p class="muted">${published?'Customer-ready published menu QR':'Owner-browser preview QR only'}</p></div><button class="btn secondary" id="previewQr">Preview Menu</button></div>${published?'':`<div class="publish-warning"><strong>This QR works only on this browser.</strong><span>Open Publish & Setup, download the config and add its public HTTPS URL before sharing a customer QR.</span><button class="btn small" id="openPublishSetup">Open Publish & Setup</button></div>`}<article class="card qr-card"><h2>${html(r.name)}</h2><p>${published?'Scan to View Published Menu':'Local Preview'}</p><div class="qr-wrap"><div id="qrcode"></div></div><div class="actions" style="margin-top:18px"><button class="btn" id="copyQr">Copy Menu Link</button><button class="btn secondary" onclick="window.print()">Print</button></div><small class="muted" style="margin-top:16px">Powered by Gravity58 Digital Menu</small></article>`;try{new QRCode($('#qrcode'),{text:url,width:220,height:220})}catch{$('#qrcode').innerHTML='<div style="color:#111;padding:70px 25px">QR library requires internet once</div>'}$('#copyQr').onclick=()=>navigator.clipboard?.writeText(url).then(()=>toast('Menu link copied'));$('#previewQr').onclick=()=>published?window.open(url,'_blank'):location.hash=`menu&restaurant=${r.id}`;$('#openPublishSetup')?.addEventListener('click',()=>{view='publish';renderShell()})}
...
function renderPublicMenu(){
  ...
  const configUrl=params.get('config')||'',published=!!configUrl;
  if(published&&remoteMenuSource!==configUrl){loadRemoteMenuConfig(configUrl);return}
  const rid=published?remoteMenuConfig?.restaurant?.id:(params.get('restaurant')||state.activeRestaurantId);
  const r=published?remoteMenuConfig?.restaurant:state.restaurants.find(x=>x.id===rid);
  ...
  const cats=published?remoteMenuConfig.categories:restaurantCategories(rid),items=published?remoteMenuConfig.items:restaurantItems(rid),heroItem=items.find(i=>i.available)||items[0];
  ...
  app.innerHTML=`<main class="public-menu compact-public-menu">
    <section class="compact-menu-hero">
      <nav class="menu-nav compact-nav"><div><h2>${html(r.name)}</h2><p>${html(r.type)} · ${html(r.city)}</p>${published?'<span class="published-menu-badge">Published customer menu</span>':''}</div><a class="sponsor-mini" href="https://www.g58.in" target="_blank" rel="noopener">Sponsored by <strong>Gravity58</strong></a></nav>
