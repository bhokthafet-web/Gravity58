...
function dashboardView(){const r=activeRestaurant(),items=restaurantItems(),orders=restaurantOrders();$('#page').innerHTML=`<div class="hero"><div><span class="status-pill"><span class="dot"></span>${r.open?'Open':'Closed'}</span><h1 style="margin:12px 0 6px">${r.logo} ${r.name}</h1><p>${r.description||r.type+' in '+r.city}</p></div><div class="actions"><button class="btn secondary" id="previewMenu">Preview Menu</button><button class="btn secondary" id="openOrders">Open Orders</button><button class="btn" id="openCsv">Manage Menu</button></div></div><div class="grid stats">${metric('Categories',restaurantCategories().length)}${metric('Menu Items',items.length)}${metric('Pending Orders',orders.filter(x=>['Pending','Payment Verification'].includes(x.status)).length)}${metric('Today Revenue',money(orders.filter(x=>x.status==='Completed').reduce((a,b)=>a+b.total,0)))}</div>`;
  $('#previewMenu').onclick=()=>{location.href=cloudCustomerMenuUrl(r)};$('#openCsv').onclick=()=>{view='menu';renderShell()};$('#openOrders').onclick=()=>{view=menuFeature('ordersEnabled')?'orders':'pricing';renderShell();if(view==='pricing')toast('Request Standard or Premium activation to receive orders')};}
...
function renderPublicMenu(){
  ...
  app.innerHTML=`<main class="public-menu compact-public-menu">
    ...
    <section class="compact-menu-hero">
      <nav class="menu-nav compact-nav"><div>${published?'<span class="published-menu-badge">Published customer menu</span>':cloudMenu?'<span class="published-menu-badge">Live account menu</span>':''}${r.premiumFeatures?'<button class="meal-subscription-entry" id="openMealSubscriptions" type="button">Premium Customer Access</button>':''}</div><a class="sponsor-mini" href="https://www.g58.in" target="_blank" rel="noopener">Sponsored by <strong>Gravity58</strong></a></nav>
    ...
    ${r.premiumFeatures?`<section class="premium-customer-access-card"><div><span class="eyebrow">PREMIUM CUSTOMER PORTAL</span><h2>Schedule orders & manage meal subscriptions</h2><p>Login is required only for scheduled orders and subscriptions. Regular immediate orders do not require a customer account.</p></div><button class="btn" id="openPremiumCustomerPortal" type="button">Login / Create Account</button></section>`:''}
  ...
  $('#openMealSubscriptions')?.addEventListener('click',premiumPortal);
  $('#openPremiumCustomerPortal')?.addEventListener('click',premiumPortal);
...
async function openCustomerSubscriptionPortal(restaurant,ownerId){
  ...
  modal('Premium Customer Access',`<div class="customer-subscription-auth"><div class="premium-account-purpose"><strong>For scheduled orders and meal subscriptions only</strong><p>Use this account to schedule a paid order or view your restaurant meal subscriptions. You can place a regular immediate order without creating an account.</p></div><form id="mealCustomerLogin"><div class="field"><label>Email</label><input name="email" type="email" required autocomplete="email"></div><div class="field"><label>Password</label><input name="password" type="password" minlength="6" required autocomplete="current-password"></div><button class="btn full">Login for Schedule & Subscriptions</button></form><div class="subscription-auth-divider">New customer?</div><form id="mealCustomerRegister"><div class="field"><label>Name</label><input name="name" required autocomplete="name"></div><div class="field"><label>Email</label><input name="email" type="email" required autocomplete="email"></div><div class="field"><label>Password</label><input name="password" type="password" minlength="6" required autocomplete="new-password"></div><button class="btn secondary full">Create Schedule Account</button></form><button class="link-btn full" id="mealCustomerForgot">Forgot Password</button></div>`,()=>{
...
async function renderCustomerSubscriptions(restaurant,ownerId,account){
  ...
  modal(`${restaurant.name} Premium Customer Dashboard`,`<div class="customer-subscription-portal"><div class="subscription-customer-head"><div><span class="eyebrow">SCHEDULE & SUBSCRIPTION ACCOUNT</span><h3>${html(account.name||account.email)}</h3><small>${html(account.email)}</small></div><button class="btn small secondary" id="mealCustomerLogout">Sign out</button></div><div class="customer-dashboard-stats"><article><strong>${activeScheduled.length}</strong><span>Active scheduled orders</span></article><article><strong>${records.filter(row=>row.status==='Active').length}</strong><span>Active subscriptions</span></article><article><strong>${scheduledOrders.length}</strong><span>Schedule history</span></article></div><section class="premium-schedule-ready"><div><strong>Schedule and pay for a menu item</strong><p>Add dishes to your cart, pay using the restaurant UPI or payment link, upload the receipt and choose the preparation time.</p></div><button class="btn small" id="continuePremiumOrder">${customerCart.length?'Schedule Current Cart':'Choose Menu Items'}</button></section><section class="customer-scheduled-orders"><div class="customer-dashboard-title"><h3>Your scheduled orders</h3><button class="link-btn" id="refreshCustomerDashboard">Refresh</button></div>${scheduledCards||empty('No scheduled orders yet. Choose menu items to create your first schedule.')}</section>${records.length?`<section class="customer-subscription-history"><h3>Your subscriptions</h3>${records.map(row=>`<article><strong>${html(row.planName)}</strong><span class="chip">${html(row.status||'Requested')}</span><p>${Number(row.mealsDelivered||0)} meal(s) delivered${row.nextScheduledMeal?` · Next: ${new Date(row.nextScheduledMeal).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'})}`:''}</p>${row.ownerNote?`<small>${html(row.ownerNote)}</small>`:''}</article>`).join('')}</section>`:''}<section class="customer-plan-section"><h3>Available meal subscriptions</h3><div class="pricing-grid customer-meal-plans">${plans.map(plan=>`<article class="card meal-plan-card"><span>${html(plan.periodLabel||'MEAL PLAN')}</span><h3>${html(plan.name)}</h3><div class="pricing-amount">${money(plan.price)}</div><p>${html(plan.description||'')}</p><div class="chips"><span class="chip">${Number(plan.meals)||0} meals</span></div><button class="btn full" data-customer-subscribe="${plan.id}">${plan.paymentLink?'Pay & Subscribe':'Request Subscription'}</button></article>`).join('')||empty('No meal subscription plans are currently available.')}</div></section></div>`,()=>{
    $('#mealCustomerLogout').onclick=async()=>{await Gravity58Ads.logout();closeModal();toast('Customer signed out')};
    $('#continuePremiumOrder').onclick=()=>{closeModal();if(customerCart.length)setTimeout(()=>openCart(restaurant),0);else{document.querySelector('#menu-list')?.scrollIntoView({behavior:'smooth'});toast('Add items, then open your cart to schedule the order')}};
    $('#refreshCustomerDashboard').onclick=()=>{closeModal();renderCustomerSubscriptions(restaurant,ownerId,account)};
    $$('[data-track-customer-order]').forEach(button=>button.onclick=()=>{const row=scheduledOrders.find(order=>(order.id||order.$id)===button.dataset.trackCustomerOrder);if(!row)return;state.orders=(state.orders||[]).filter(order=>order.id!==(row.id||row.$id));state.orders.unshift({...row,id:row.id||row.$id,cloudOwnerId:row.cloudOwnerId||ownerId});save();closeModal();location.hash=`track&order=${encodeURIComponent(row.id||row.$id)}`;renderTrack()});
    $$('[data-customer-subscribe]').forEach(button=>button.onclick=async()=>{const plan=plans.find(row=>row.id===button.dataset.customerSubscribe);if(!plan)return;const existing=records.find(row=>row.planId===plan.id&&!['Cancelled','Rejected'].includes(row.status));if(existing)return toast('You already have this subscription request');button.disabled=true;const request={ownerId,restaurantId:restaurant.id,restaurantName:restaurant.name,customerAccountId:account.$id,customerName:account.name||account.email.split('@')[0],customerEmail:account.email,planId:plan.id,planName:plan.name,totalMeals:Number(plan.meals)||0,mealsDelivered:0,price:Number(plan.price)||0,paymentLink:plan.paymentLink||'',status:plan.paymentLink?'Payment Pending':'Requested',createdAt:now()};const popup=plan.paymentLink?window.open('about:blank','_blank'):null;try{const permissions=Gravity58Ads.userPermissionSet?.([account.$id,ownerId])||Gravity58Ads.collaborativePermissionSet(account.$id);await Gravity58Ads.create(ownerSubscriptionKind(ownerId),request,undefined,permissions);if(popup)popup.location.href=plan.paymentLink;closeModal();toast('Subscription request sent to the restaurant');renderCustomerSubscriptions(restaurant,ownerId,account)}catch(error){popup?.close();button.disabled=false;toast(error.message||'Could not create subscription request')}});
  });
}
