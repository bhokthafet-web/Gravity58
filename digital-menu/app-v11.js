...
function ordersView(){
  ...
  $('#page').innerHTML=`<div class="section-head"><div><h1>Live Orders <small class="order-reset-copy">(${menuFeature('permanentOrders')?'Premium: history available for up to 1 year':`You're using freeware — orders reset in <span id="orderResetCountdown">${orderResetCountdown()}</span>`})</small></h1><p class="muted">Instant order board · ${active.length} active token(s) always pinned · history: ${html(ownerPeriodLabel())}</p></div><div class="orders-header-actions">${posButton}<span class="status-pill live-sync-pill"><span class="dot"></span> Live sync & sound alerts</span></div></div><div class="order-controls"><div class="tabs">${statuses.map(s=>`<button class="btn small ${s===ownerOrderStatusFilter?'':'secondary'}" data-tab="${s}">${s}</button>`).join('')}</div></div><div id="ordersGrid" class="orders-board">${ordersBoardMarkup(visible,`No ${ownerOrderStatusFilter==='All'?'':ownerOrderStatusFilter.toLowerCase()+' '}orders in this period`)}</div>`;
