  ...
  modal(id?'Edit Restaurant':first?'Create Your First Restaurant':'Add Restaurant',restaurantForm(r),()=>{
    ...
    form.onsubmit=async e=>{
      ...
      try{
        ...
        await persistCloudMenu(saved.id);Gravity58Ads?.upsertSlot({id:saved.id,restaurantKey:saved.restaurantKey,name:saved.name,city:saved.city,active:true}).catch(()=>{});if(previewUrl)URL.revokeObjectURL(previewUrl);closeModal();renderShell();toast('Restaurant saved to G58 Cloud')
...
function menuView(){const cats=restaurantCategories(),items=restaurantItems();$('#page').innerHTML=`<div class="section-head"><div><h1>Menu CSV</h1><p class="muted">Download the template, add or update dishes in the spreadsheet, then import the CSV.</p></div><div class="actions"><button class="btn secondary" id="downloadMenuCsv">Download CSV Template</button><button class="btn secondary" id="chooseMenuImages">Choose Food Images</button><button class="btn" id="importMenuCsv">Import Menu CSV</button><input id="menuImageFiles" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden><input id="menuCsvFile" type="file" accept=".csv,text/csv" hidden></div></div><div class="cloud-menu-note"><strong>Appwrite menu images</strong><span>Menu items are created only from CSV. Put each local image name in the image_file column and choose matching JPG, PNG or WebP files of 100 KB or less. G58 uploads them directly to Appwrite.</span><small id="imageSelectionStatus">No food images selected</small></div><div class="toolbar"><input id="itemSearch" placeholder="Search menu items"><select id="catFilter"><option value="">All categories</option>${cats.map(c=>`<option value="${c.id}">${html(c.name)}</option>`).join('')}</select></div><div class="grid menu-grid" id="menuGrid">${items.map(menuCard).join('')||empty('Import your first menu CSV')}</div>`;$('#downloadMenuCsv').onclick=downloadMenuCsvTemplate;$('#chooseMenuImages').onclick=()=>$('#menuImageFiles').click();$('#menuImageFiles').onchange=e=>{$('#imageSelectionStatus').textContent=e.target.files.length?`${e.target.files.length} food image${e.target.files.length===1?'':'s'} ready for the CSV import`:'No food images selected'};$('#importMenuCsv').onclick=()=>$('#menuCsvFile').click();$('#menuCsvFile').onchange=e=>importMenuCsvFile(e.target.files[0],[...$('#menuImageFiles').files]);$('#itemSearch').oninput=filterItems;$('#catFilter').onchange=filterItems;bindMenuActions()}
function menuCard(i){const c=state.categories.find(x=>x.id===i.categoryId);return `<article class="card menu-item" data-name="${html(i.name.toLowerCase())}" data-cat="${i.categoryId}"><span class="availability chip">${i.available?'Available':'Out of stock'}</span><div class="food-img menu-owner-image">${imageMarkup(localMediaSource(i,'imageKey','imageData'),i.name?.[0]||'G','menu-owner-photo')}</div><div class="chips"><span class="chip">${html(c?.name||'Uncategorised')}</span><span class="chip">${html(i.type)}</span></div><h3>${html(i.name)}</h3><p class="muted">${html(i.description)}</p>${i.prepareInstructionsEnabled?'<span class="chip instruction-enabled-chip">Preparation instructions enabled</span>':''}<div class="price">${money(i.price)}</div><div class="actions" style="margin-top:12px"><button class="btn small ${i.available?'red':'green'}" data-stock="${i.id}">${i.available?'Out of stock':'Make available'}</button><button class="btn small red" data-item-delete="${i.id}">Remove</button></div></article>`}
...
async function importMenuCsvFile(file,imageFiles=[]){if(!file)return;if(file.size>2*1024*1024)return toast('CSV file must be below 2 MB');const categoriesBefore=structuredClone(state.categories),itemsBefore=structuredClone(state.items),uploadedFiles=[],oldFiles=[];try{const rows=Gravity58MenuData.parseMenuCsv(await file.text());if(rows.length>100)throw new Error('Import up to 100 menu items at a time');imageFiles.forEach(entry=>Gravity58Ads.validateMenuImage(entry));const images=new Map(imageFiles.map(entry=>[entry.name.trim().toLowerCase(),entry]));for(const row of rows){if(row.image_file&&!images.has(row.image_file.trim().toLowerCase()))throw new Error(`Choose the image file named ${row.image_file}`)}let created=0,updated=0;for(const row of rows){const categoryName=row.category.trim().slice(0,80),normalisedCategory=categoryName.toLowerCase();let category=restaurantCategories().find(entry=>entry.name.trim().toLowerCase()===normalisedCategory);if(!category){category={id:uid('cat'),restaurantId:state.activeRestaurantId,name:categoryName};state.categories.push(category)}let item=restaurantItems().find(entry=>entry.categoryId===category.id&&entry.name.trim().toLowerCase()===row.item_name.trim().toLowerCase());if(item)updated++;else{item={id:uid('item'),restaurantId:state.activeRestaurantId,available:true};state.items.push(item);created++}Object.assign(item,{categoryId:category.id,name:row.item_name.trim().slice(0,120),description:(row.description||'').trim().slice(0,500),price:Number(row.price),type:String(row.food_type||'Veg').trim().toLowerCase().includes('non')?'Non-Veg':'Veg',available:Gravity58MenuData.csvBoolean(row.available,true),prep:Math.max(0,Number(row.preparation_minutes)||0),prepareInstructionsEnabled:Gravity58MenuData.csvBoolean(row.preparation_instructions,false)});const selectedImage=images.get(String(row.image_file||'').trim().toLowerCase());if(selectedImage){const upload=await uploadMenuImage(selectedImage,item.name);if(item.imageFileId)oldFiles.push(item.imageFileId);item.imageUrl=upload.mediaUrl;item.imageFileId=upload.fileId;uploadedFiles.push(upload.fileId)}}await persistCloudMenu();oldFiles.filter(Boolean).forEach(fileId=>Gravity58Ads.removeMenuMedia(fileId).catch(()=>{}));menuView();toast(`${created} created, ${updated} updated from CSV`)}catch(error){state.categories=categoriesBefore;state.items=itemsBefore;uploadedFiles.filter(Boolean).forEach(fileId=>Gravity58Ads.removeMenuMedia(fileId).catch(()=>{}));toast(error.message||'Could not import menu CSV')}finally{$('#menuCsvFile')&&($('#menuCsvFile').value='')}}
...
function publishSetupView(){
  const r=activeRestaurant(),url=cloudCustomerMenuUrl(r);$('#page').innerHTML=`<div class="section-head"><div><h1>Share Menu</h1><p class="muted">This customer link always opens the latest menu from G58 Cloud.</p></div></div><article class="card cloud-share-card"><div><span class="publish-step">LIVE</span><h2>${html(r.name)} customer menu</h2><p>Restaurant details, menu items, images and availability update automatically. No hosting or cloud configuration is required.</p><a href="${html(url)}" target="_blank" rel="noopener">${html(url)}</a><div class="actions"><button class="btn" id="copyCloudMenuLink">Copy Customer Link</button><a class="btn secondary" href="${html(url)}" target="_blank" rel="noopener">Open Customer Menu</a></div></div><div class="qr-wrap"><div id="cloudMenuQr"></div></div></article><article class="card"><h2>Update menu by CSV</h2><p class="muted">Download the template and import up to 100 dishes at a time.</p><button class="btn secondary" id="openMenuSetup">Open Menu CSV</button></article>`;try{new QRCode($('#cloudMenuQr'),{text:url,width:220,height:220})}catch{$('#cloudMenuQr').textContent='QR unavailable'}$('#copyCloudMenuLink').onclick=()=>navigator.clipboard?.writeText(url).then(()=>toast('Customer link copied'));$('#openMenuSetup').onclick=()=>{view='menu';renderShell()}
}
function qrView(){const r=activeRestaurant(),url=cloudCustomerMenuUrl(r);$('#page').innerHTML=`<div class="section-head"><div><h1>QR Codes</h1><p class="muted">Live customer menu QR from G58 Cloud</p></div><button class="btn secondary" id="previewQr">Preview Menu</button></div><article class="card qr-card"><h2>${html(r.name)}</h2><p>Scan to View Menu</p><div class="qr-wrap"><div id="qrcode"></div></div><div class="actions" style="margin-top:18px"><button class="btn" id="copyQr">Copy Menu Link</button><button class="btn secondary" onclick="window.print()">Print</button></div><small class="muted" style="margin-top:16px">Powered by Gravity58 Digital Menu</small></article>`;try{new QRCode($('#qrcode'),{text:url,width:220,height:220})}catch{$('#qrcode').innerHTML='<div style="color:#111;padding:70px 25px">QR unavailable</div>'}$('#copyQr').onclick=()=>navigator.clipboard?.writeText(url).then(()=>toast('Menu link copied'));$('#previewQr').onclick=()=>window.open(url,'_blank')}
  ...
  modal('Your Cart',`<div id="checkoutPanel">
    ...
    button.addEventListener('click',async()=>{
      ...
      try{
        ...
        fail('Order could not be saved to G58 Cloud. Please check the connection and try again.');
      }
    });
  });
}
