...
async function secureSubscriptionAction(action,row,extra={}){
  ...
  if(action==='send-subscription-link'){if(!row.paymentLink)throw new Error('Add a secure payment link to this plan before sending it.');return Gravity58Ads.update(ownerSubscriptionKind(ownerId),subscriptionId,{status:'Payment Link Sent',paymentLinkSentAt:now(),updatedAt:now()})}
...
function drawSubscriptionsView(){
  ...
  const subscriberCards=subscribers.map(row=>{const id=row.id||row.$id,delivered=Number(row.mealsDelivered||0),total=Number(row.totalMeals||0),progress=total?Math.min(100,Math.round(delivered/total*100)):0;let workflow='';
    if(row.status==='Requested')workflow=`<button class="btn small subscription-owner-primary" data-send-sub-link="${html(id)}">Send payment link</button>`;
  ...
  $$('[data-send-sub-link]').forEach(button=>button.onclick=async()=>{const row=subscribers.find(item=>(item.id||item.$id)===button.dataset.sendSubLink);if(!row)return;const plan=plans.find(item=>item.id===row.planId);if(plan)Object.assign(row,{paymentLink:plan.paymentLink||row.paymentLink,deliveryDays:normaliseDeliveryDays(plan.deliveryDays),deliveryTime:plan.deliveryTime||row.deliveryTime});button.disabled=true;try{Object.assign(row,await secureSubscriptionAction('send-subscription-link',row));drawSubscriptionsView();toast('Payment link sent to the customer dashboard')}catch(error){button.disabled=false;toast(error.message||'Could not send payment link')}});
...
function publicAdSection(r){
  ...
  if(ad)return `<section class="public-ad active-ad ad-cover-placement"><div class="ad-art">${adMediaMarkup(ad,'public-ad-media')}</div><a class="ad-get-offer" href="${html(ad.destinationUrl||'#')}" target="_blank" rel="noopener">Get Offer</a><small class="ad-expiry ad-expiry-badge" data-ad-expiry="${html(ad.expiresAt||'')}" data-ad-lifetime="${ad.lifetime===true?'true':'false'}">${adTimeLeft(ad)}</small></section>`;
...
function renderPublicMenu(){
  ...
  app.innerHTML=`<main class="public-menu compact-public-menu">
    ...
    <aside class="header-ad-panel">
      ...
      ${ad?`<div class="header-active-ad creative-${html(ad.creativeStyle||'spotlight')}"><div class="header-ad-art">${adMediaMarkup(ad,'header-ad-media')}</div><a class="ad-get-offer" href="${html(ad.destinationUrl||'#')}" target="_blank" rel="noopener">Get Offer</a><small class="ad-expiry ad-expiry-badge" data-ad-expiry="${html(ad.expiresAt||'')}" data-ad-lifetime="${ad.lifetime===true?'true':'false'}">${adTimeLeft(ad)}</small></div>`:`<div class="header-ad-placeholder"><span class="ad-label">GRAVITY58 AD SPACE</span><div class="header-ad-art"><span class="media-fallback">G58</span></div><div><h3>Put your brand beside the menu</h3><p>Best image: 1080 × 1350 px (4:5)</p></div></div>`}
    </aside>
    </section>
