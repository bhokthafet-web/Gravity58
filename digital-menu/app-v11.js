...
async function persistCloudOrder(order){
  ...
  if(order.paymentMethod==='counter'&&validCustomerPhone(order.phone)){
    ...
    const mergeable=row=>row&&row.restaurantId===order.restaurantId&&row.customerAccountId===order.customerAccountId&&normaliseCustomerPhone(row.phone)===normaliseCustomerPhone(order.phone)&&row.paymentMethod==='counter'&&row.orderDay===orderDay()&&['Pending','Accepted','Preparing','Ready'].includes(row.status);
...
function dashboardView(){const r=activeRestaurant(),items=restaurantItems(),orders=ordersForOwnerPeriod(restaurantOrders()),period=ownerPeriodLabel(),completed=orders.filter(x=>x.status==='Completed'),posAction=menuFeature('posPremium')&&isCloudMenuSession()?`<a class="btn premium-pos-button" href="${premiumPosUrl(r)}">Open Premium POS</a>`:'';$('#page').innerHTML=`<div class="hero"><div><span class="status-pill"><span class="dot"></span>${r.open?'Open':'Closed'}</span><h1 style="margin:12px 0 6px">${r.logo} ${r.name}</h1><p>${r.description||r.type+' in '+r.city}</p></div><div class="actions"><button class="btn secondary" id="previewMenu">Preview Menu</button><button class="btn secondary" id="openOrders">Open Orders</button><button class="btn" id="openCsv">Manage Menu</button>${posAction}</div></div>${menuFeature('posPremium')&&isCloudMenuSession()?`<article class="card premium-pos-sync-card"><div><span class="eyebrow">RESTAURANT-SYNCED BILLING</span><h2>Premium POS is ready</h2><p><strong>${html(r.name)}</strong> has the complete G58 POS workspace with its menu, inventory, counter bills and live Digital Menu orders kept together.</p></div><a class="btn premium-pos-button" href="${premiumPosUrl(r)}">Open Restaurant POS →</a></article>`:''}<div class="dashboard-filter-row"><div><h2>${html(period)} overview</h2><p class="muted">Order totals and the list below follow this period.</p></div>${ownerPeriodControls('Filter restaurant dashboard')}</div><div class="grid stats">${metric('Categories',restaurantCategories().length)}${metric('Menu Items',items.length)}${metric('Orders',orders.length)}${metric('Revenue',money(completed.reduce((a,b)=>a+b.total,0)))}</div><div class="section-head"><div><h2>Orders for ${html(period)}</h2><p class="muted">${orders.length} order${orders.length===1?'':'s'} in the selected period</p></div></div>${orderCompactTable(orders.slice(0,12),{actions:false,emptyText:'No orders in this period'})}`;
...
function ordersBoardMarkup(orders,emptyText='No orders yet'){if(!orders.length)return empty(emptyText);const featured=orders.slice(0,3),history=orders.slice(3);return `<div class="featured-order-grid">${featured.map(orderCard).join('')}</div>${history.length?`<div class="section-head compact-order-heading"><div><h2>More orders</h2><p class="muted">Order 4 onward is shown in a compact list.</p></div><span class="chip">${history.length} more</span></div>${orderCompactTable(history)}`:''}`}
function ordersView(){
