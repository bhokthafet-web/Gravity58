...
function optimizeLocalImage(file,{maxWidth=960,quality=.78}={}){
  return new Promise((resolve,reject)=>{
    ...
    reader.onload=()=>{const img=new Image();img.onerror=()=>reject(new Error('Invalid image file'));img.onload=()=>{const scale=Math.min(1,maxWidth/img.width),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(img.width*scale));canvas.height=Math.max(1,Math.round(img.height*scale));canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL('image/webp',quality))};img.src=reader.result};reader.readAsDataURL(file);
  });
}
...
function restaurantVisual(r,className=''){return imageMarkup(r.logoImage,r.name?.trim()?.[0]||'G',className)}
...
function menuCard(i){const c=state.categories.find(x=>x.id===i.categoryId);return `<article class="card menu-item" data-name="${html(i.name.toLowerCase())}" data-cat="${i.categoryId}"><span class="availability chip">${i.available?'Available':'Out of stock'}</span><div class="food-img menu-owner-image">${imageMarkup(i.imageData,i.emoji||i.name?.[0]||'G','menu-owner-photo')}</div><div class="chips"><span class="chip">${html(c?.name||'Uncategorised')}</span><span class="chip">${html(i.type)}</span></div><h3>${html(i.name)}</h3><p class="muted">${html(i.description)}</p>${i.prepareInstructionsEnabled?'<span class="chip instruction-enabled-chip">Preparation instructions enabled</span>':''}<div class="price">${money(i.price)}</div><div class="actions" style="margin-top:12px"><button class="btn small secondary" data-item-edit="${i.id}">Edit</button><button class="btn small ${i.available?'red':'green'}" data-stock="${i.id}">${i.available?'Out of stock':'Make available'}</button><button class="btn small red" data-item-delete="${i.id}">Delete</button></div></article>`}
...
function itemForm(i={}){const cats=restaurantCategories(),category=state.categories.find(c=>c.id===i.categoryId);return `<form id="itemForm"><datalist id="categorySuggestions">${cats.map(c=>`<option value="${html(c.name)}">`).join('')}</datalist><div class="form-grid"><div class="field"><label>Item name</label><input name="name" value="${html(i.name||'')}" required></div><div class="field"><label>Category <small>(type your own)</small></label><input name="categoryName" list="categorySuggestions" value="${html(category?.name||'')}" placeholder="Example: Starters" required></div><div class="field"><label>Price</label><input name="price" type="number" min="0" value="${i.price||''}" required></div><div class="field"><label>Food type</label><select name="type"><option ${i.type==='Veg'?'selected':''}>Veg</option><option ${i.type==='Non-Veg'?'selected':''}>Non-Veg</option></select></div><div class="field local-image-field"><label>Menu item image <small>(stored only in this browser)</small></label><input name="imageFile" type="file" accept="image/jpeg,image/png,image/webp"><div class="image-preview menu-item-preview">${imageMarkup(i.imageData,i.emoji||i.name?.[0]||'G','menu-preview-image')}</div></div><div class="field"><label>Preparation minutes</label><input name="prep" type="number" min="0" value="${i.prep||15}"></div><div class="field"><label>Allow customer preparation instructions</label><select name="prepareInstructionsEnabled"><option value="true" ${i.prepareInstructionsEnabled?'selected':''}>Enabled</option><option value="false" ${!i.prepareInstructionsEnabled?'selected':''}>Disabled</option></select></div></div><div class="field"><label>Description</label><textarea name="description">${html(i.description||'')}</textarea></div><button class="btn full">Save Menu Item</button></form>`}
function openItemForm(id=null){const i=id?state.items.find(x=>x.id===id):{};modal(id?'Edit Menu Item':'Add Menu Item',itemForm(i),()=>{const form=$('#itemForm'),file=form.imageFile,preview=$('.menu-item-preview');file.onchange=async()=>{try{const data=await optimizeLocalImage(file.files[0]);if(data)preview.innerHTML=imageMarkup(data,'G','menu-preview-image')}catch(error){file.value='';toast(error.message)}};form.onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),selectedFile=fd.get('imageFile');fd.delete('imageFile');const d=Object.fromEntries(fd),categoryName=d.categoryName.trim();delete d.categoryName;let category=restaurantCategories().find(c=>c.name.trim().toLowerCase()===categoryName.toLowerCase());if(!category){category={id:uid('cat'),restaurantId:state.activeRestaurantId,name:categoryName};state.categories.push(category)}const button=e.submitter;button.disabled=true;button.textContent='Saving…';try{const imageData=selectedFile?.size?await optimizeLocalImage(selectedFile):i.imageData||'';const values={...d,categoryId:category.id,imageData,price:+d.price,prep:+d.prep,prepareInstructionsEnabled:d.prepareInstructionsEnabled==='true'};if(id)Object.assign(i,values);else state.items.push({id:uid('item'),restaurantId:state.activeRestaurantId,...values,available:true});save();closeModal();menuView();toast('Menu item saved')}catch(error){button.disabled=false;button.textContent='Save Menu Item';toast(error.message||'Could not save menu item')}}})}
...
function renderPublicMenu(){
  ...
  app.innerHTML=`<main class="public-menu compact-public-menu">
    <section class="compact-menu-hero">
      ...
      <div class="compact-hero-layout"><div><p class="eyebrow">WELCOME TO ${html(r.name.toUpperCase())}</p><h1>Delicious Food<br>Is Waiting For You</h1><p>${html(r.description)}</p><div class="compact-details"><span>📍 ${html(r.address||r.city)}</span><span>☎ ${html(r.phone||'Contact restaurant')}</span><span class="open-tag">${r.open?'Open now':'Closed'}</span></div></div><div class="compact-hero-dish">${imageMarkup(heroItem?.imageData||r.logoImage,heroItem?.emoji||r.logo||'G','compact-hero-photo')}</div></div>
    </section>
    <section class="menu-workspace" id="menu-list">
      <section class="focused-menu-panel">
        <nav class="category-chip-strip" aria-label="Menu categories">${cats.map(c=>`<button class="category-chip ${c.id===activeCategory?.id?'active':''}" data-cat-select="${c.id}">${html(c.name)}</button>`).join('')}</nav>
        <header class="focused-menu-heading"><div><p class="eyebrow">FOOD MENU</p><h2>${html(activeCategory?.name||'Menu')}</h2></div><span>${items.filter(i=>i.categoryId===activeCategory?.id).length} items</span></header>
...
function publicItem(i){
  ...
  return `<article class="poster-menu-item ${!i.available?'sold-out':''}"><div class="poster-food-thumb">${imageMarkup(i.imageData,i.emoji||i.name?.[0]||'G','poster-food-photo')}</div><div class="poster-item-copy"><div class="poster-title-row"><h3>${html(i.name)}</h3><span class="dot-leader"></span><strong>${money(i.price)}</strong></div><p>${html(i.description)}</p>${i.prepareInstructionsEnabled?`<label class="prepare-instruction-trigger"><input type="checkbox" data-prepare-item="${i.id}" ${instruction?'checked':''}><span>Prepare instructions</span></label>${instruction?`<div class="saved-prep-instruction">${html(instruction)}</div>`:''}`:''}<div class="poster-meta"><span class="food-dot ${i.type==='Veg'?'veg':'nonveg'}"></span><span>${html(i.type)}</span><span>•</span><span>${i.prep} min</span></div></div><div class="poster-action">${i.available?(qty?`<div class="quantity-control"><button data-qty-action="minus" data-item="${i.id}" aria-label="Remove one">−</button><strong>${qty}</strong><button data-qty-action="plus" data-item="${i.id}" aria-label="Add one">+</button></div>`:`<button class="btn small add-item" data-qty-action="plus" data-item="${i.id}">Add</button>`):'<span class="out-label">Out</span>'}</div></article>`
}
