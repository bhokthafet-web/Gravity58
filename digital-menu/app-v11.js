...
function drawSubscriptionsView(){
  ...
  $('#page').innerHTML=`<section class="subscription-owner-hero"><div><span class="eyebrow">PROFIT-STYLE MEAL MANAGEMENT</span><h1>Meal Subscriptions</h1><p>Create branded plans, activate customers, schedule their next meal and record every delivery.</p></div><button class="btn" id="addMealPlan">+ Create Subscription Card</button></section><div class="grid stats">${metric('Active Plans',plans.filter(row=>row.active!==false).length)}${metric('Subscribers',subscribers.length)}${metric('Active Subscribers',subscribers.filter(row=>row.status==='Active').length)}${metric('Meals Delivered',subscribers.reduce((sum,row)=>sum+Number(row.mealsDelivered||0),0))}</div><div class="section-head"><div><h2>Customer-facing plans</h2><p class="muted">Each plan’s type, colours and cover image are editable.</p></div></div><div class="pricing-grid owner-meal-plans">${plans.map(plan=>subscriptionPlanCard(plan,{owner:true})).join('')||empty('Create your first customer subscription card')}</div><div class="section-head"><div><h2>Subscribed customers</h2><p class="muted">Manage activation, meal balance and the next scheduled service.</p></div></div><div class="subscriber-management-grid">${subscribers.map(row=>{const delivered=Number(row.mealsDelivered||0),total=Number(row.totalMeals||0),progress=total?Math.min(100,Math.round(delivered/total*100)):0;return `<article class="card subscriber-management-card status-${html(String(row.status||'requested').toLowerCase().replace(/\s+/g,'-'))}"><div class="subscriber-card-head"><div><span class="eyebrow">${html(row.planType||row.planName||'MEAL PLAN')}</span><h3>${html(row.customerName||'Customer')}</h3><small>${html(row.customerEmail||'')}</small></div><span class="chip">${html(row.status||'Requested')}</span></div><div class="subscriber-meal-progress"><div><span>Meals delivered</span><strong>${delivered} / ${total||'∞'}</strong></div><progress max="100" value="${progress}"></progress></div><p><strong>Next meal:</strong> ${row.nextScheduledMeal?new Date(row.nextScheduledMeal).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'}):'Not scheduled'}</p>${row.ownerNote?`<p class="subscriber-note">${html(row.ownerNote)}</p>`:''}<div class="actions"><button class="btn small green" data-deliver-meal="${html(row.id||row.$id)}">Meal Delivered</button><button class="btn small secondary" data-manage-sub="${html(row.id||row.$id)}">Manage</button></div></article>`}).join('')||empty('No customer subscriptions yet.')}</div>`;
...
async function openCustomerSubscriptionPortal(restaurant,ownerId){
  ...
  if(account?.email)return openCustomerSubscriptionDashboard(restaurant,ownerId,account);
  modal('Premium Customer Access',`<div class="customer-subscription-auth"><div class="premium-account-purpose"><strong>For scheduled orders and meal subscriptions only</strong><p>Use this account to schedule a paid order or view your restaurant meal subscriptions. You can place a regular immediate order without creating an account.</p></div><form id="mealCustomerLogin"><div class="field"><label>Email</label><input name="email" type="email" required autocomplete="email"></div><div class="field"><label>Password</label><input name="password" type="password" minlength="6" required autocomplete="current-password"></div><button class="btn full">Login for Schedule & Subscriptions</button></form><div class="subscription-auth-divider">New customer?</div><form id="mealCustomerRegister"><div class="field"><label>Name</label><input name="name" required autocomplete="name"></div><div class="field"><label>Email</label><input name="email" type="email" required autocomplete="email"></div><div class="field"><label>Password</label><input name="password" type="password" minlength="6" required autocomplete="new-password"></div><button class="btn secondary full">Create Schedule Account</button></form><button class="link-btn full" id="mealCustomerForgot">Forgot Password</button></div>`,()=>{
    $('#mealCustomerLogin').onsubmit=async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.target)),button=event.submitter;button.disabled=true;try{await Gravity58Ads.login(values.email.trim(),values.password);const user=await Gravity58Ads.currentUser();closeModal();openCustomerSubscriptionDashboard(restaurant,ownerId,user)}catch(error){button.disabled=false;toast(error.message||'Could not sign in')}};
    $('#mealCustomerRegister').onsubmit=async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.target)),button=event.submitter;button.disabled=true;try{const user=await Gravity58Ads.register(values.email.trim(),values.password,values.name);closeModal();openCustomerSubscriptionDashboard(restaurant,ownerId,user)}catch(error){button.disabled=false;toast(error.message||'Could not create customer account')}};
...
function customerMenuHash(restaurant,ownerId){return `menu&cloud=${encodeURIComponent(restaurant.cloudRecordId||restaurant.id)}&owner=${encodeURIComponent(ownerId)}`}
function customerSubscriptionHash(restaurant,ownerId){return `subscriptions&cloud=${encodeURIComponent(restaurant.cloudRecordId||restaurant.id)}&owner=${encodeURIComponent(ownerId)}`}
...
async function renderCustomerSubscriptions(restaurant,ownerId,account){
  ...
  const subscriptionHistory=records.map(row=>{const delivered=Number(row.mealsDelivered||0),total=Number(row.totalMeals||0),progress=total?Math.min(100,Math.round(delivered/total*100)):0,next=row.nextScheduledMeal?`Next meal: ${new Date(row.nextScheduledMeal).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'})}`:'Next meal is not scheduled.';return `<article class="card customer-active-plan"><span class="eyebrow">${html(row.planType||row.planName||'MEAL PLAN')}</span><h3>${html(row.planName)}</h3><span class="chip">${html(row.status||'Requested')}</span><div class="subscriber-meal-progress"><div><span>Meals delivered</span><strong>${delivered} / ${total||'∞'}</strong></div><progress max="100" value="${progress}"></progress></div><p>${html(next)}</p>${row.ownerNote?`<small>${html(row.ownerNote)}</small>`:''}</article>`}).join('');
  ...
  $$('[data-customer-subscribe]').forEach(button=>button.onclick=async()=>{const plan=plans.find(row=>row.id===button.dataset.customerSubscribe);if(!plan)return;const existing=records.find(row=>row.planId===plan.id&&!['Cancelled','Rejected'].includes(row.status));if(existing)return toast('You already have this subscription request');button.disabled=true;const request={ownerId,restaurantId:restaurant.id,planId:plan.id,customerName:account.name||account.email.split('@')[0],customerEmail:account.email};const popup=plan.paymentLink?window.open('about:blank','_blank'):null;try{let created;const functionId=Gravity58Ads.config?.digitalOrderFunctionId;if(functionId&&Gravity58Ads.executeFunction){const result=await Gravity58Ads.executeFunction(functionId,{action:'create-subscription',subscription:request});created=result?.subscription;if(!created)throw new Error('Secure subscription service did not return the request.')}else{const permissions=Gravity58Ads.userPermissionSet?.([account.$id,ownerId])||Gravity58Ads.collaborativePermissionSet(account.$id);created=await Gravity58Ads.create(ownerSubscriptionKind(ownerId),{...request,restaurantName:restaurant.name,customerAccountId:account.$id,planName:plan.name,planType:plan.planType||plan.name,totalMeals:Number(plan.meals)||0,mealsDelivered:0,price:Number(plan.price)||0,paymentLink:plan.paymentLink||'',status:plan.paymentLink?'Payment Pending':'Requested',createdAt:now()},undefined,permissions)}if(popup)popup.location.href=plan.paymentLink;toast('Subscription request sent to the restaurant');renderCustomerSubscriptions(restaurant,ownerId,account)}catch(error){popup?.close();button.disabled=false;toast(error.message||'Could not create subscription request')}});
}
