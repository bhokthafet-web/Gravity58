  ...
  app.innerHTML=`<main class="public-menu compact-public-menu">
    ...
    <section class="menu-workspace" id="menu-list">
      <section class="focused-menu-panel">
        ...
        <div class="poster-menu-list swiggy-menu-list" id="publicMenuItems">${items.map(publicItem).join('')}</div>
...
function publicItem(i){
  ...
  const category=state.categories.find(c=>c.id===i.categoryId)?.name||'';
  return `<article class="poster-menu-item swiggy-menu-item ${!i.available?'sold-out':''}" data-category="${html(i.categoryId)}" data-type="${html(i.type)}" data-available="${i.available?'true':'false'}" data-price="${Number(i.price)||0}" data-order="${state.items.indexOf(i)}" data-search="${html(`${i.name} ${i.description||''} ${category}`.toLowerCase())}"><div class="poster-item-copy swiggy-item-copy"><div class="poster-meta"><span class="food-dot ${i.type==='Veg'?'veg':'nonveg'}"></span><span>${html(i.type)}</span><span>•</span><span>${html(category)}</span></div><div class="poster-title-row"><h3>${html(i.name)}</h3></div><strong class="swiggy-price">${money(i.price)}</strong><p>${html(i.description)}</p><div class="swiggy-item-details"><span>★ ${i.available?'Popular':'Unavailable'}</span><span>${Number(i.prep)||0} min</span></div>${i.prepareInstructionsEnabled?`<label class="prepare-instruction-trigger"><input type="checkbox" data-prepare-item="${i.id}" ${instruction?'checked':''}><span>Prepare instructions</span></label>${instruction?`<div class="saved-prep-instruction">${html(instruction)}</div>`:''}`:''}</div><div class="swiggy-food-side"><div class="poster-food-thumb">${imageMarkup(localMediaSource(i,'imageKey','imageData'),i.name?.[0]||'G','poster-food-photo')}</div><div class="poster-action">${i.available?(qty?`<div class="quantity-control"><button data-qty-action="minus" data-item="${i.id}" aria-label="Remove one">−</button><strong>${qty}</strong><button data-qty-action="plus" data-item="${i.id}" aria-label="Add one">+</button></div>`:`<button class="btn small add-item" data-qty-action="plus" data-item="${i.id}">ADD</button>`):'<span class="out-label">SOLD OUT</span>'}</div></div></article>`
}
