(function (root) {
  ...
  const MENU_CSV_TEMPLATE = [
    ...
    'Starters,"Veg Spring Rolls","Crispy rolls with vegetables",180,Veg,true,15,false,spring-rolls.jpg',
    'Main Course,"Chicken Biryani","Basmati rice with chicken",320,Non-Veg,true,25,true,chicken-biryani.jpg',
  ].join("\n");

  function normaliseHeader(value) {
    const key = String(value || "").trim().toLowerCase().replace(/[\s-]+/g, "_");
    return ({ name: "item_name", item: "item_name", type: "food_type", prep: "preparation_minutes", instructions: "preparation_instructions", image: "image_file" })[key] || key;
  }
  ...
  function parseMenuCsv(text) {
    ...
    rows.forEach((row, index) => {
      ...
      if (row.image_file && /[\\/]/.test(row.image_file)) throw new Error(`Row ${index + 2}: image_file must contain a file name only`);
    });
    return rows;
  }
