...
function siteFooter(forCustomer){
  const badge=forCustomer
    ...
    : `<a class="g58-app-badge" href="/downloads/GRAVITY58-Android-v1.8.apk" download aria-label="Download the Gravity58 Android app"><span class="g58-app-badge-icon">▶</span><span class="g58-app-badge-text"><small>Download Android</small><strong>Get G58 App</strong></span></a>`;
...
function customerPromotionTicket(promotion,decorative=false){
  ...
  return `<article class="promotion-ticket customer-ticket ${hasImage?'brand-art-ticket':''}" aria-label="${html(promotion.name)}" ${decorative?'aria-hidden="true"':''}><span class="promotion-badge">Special Offer</span>${hasImage?`<div class="promotion-ticket-image"><img src="${html(promotion.imageUrl)}" alt="${html(promotion.name)}" loading="lazy" decoding="async"></div>`:`<h3>${html(promotion.name)}</h3>`}${Number(promotion.price)>0?`<strong class="promotion-offer-price">₹${Math.round(promotion.price)}/- only</strong>`:''}${promotion.endsOn?`<small class="promotion-end-date">Offer ends ${html(formatPromotionEnd(promotion.endsOn))}</small>`:''}<div class="promotion-ticket-foot"><div class="promotion-control" data-promotion-control="${html(promotion.id)}">${promotionQuantityControl(promotion.id)}</div></div></article>`;
}
  ...
  $('#page').innerHTML=`<div class="section-head"><div><h1>Availability</h1><p class="muted">Set the days and hours customers can book, and how far ahead they're allowed to book.</p></div></div>
    <div class="card" style="max-width:560px">
      <form id="availabilityForm">
        ...
        <div class="field"><label>Slot length (minutes)</label><input name="slotDurationMinutes" type="number" min="5" step="5" value="${store.slotDurationMinutes||30}" required><small class="muted">Used when a service doesn't set its own duration.</small></div>
