...
const stayExtraKind=(ownerId)=>safeId('digit58_stay_extra_',ownerId,35);
const isGameZone=(store)=>store?.bookingMode==='game_zones'||store?.businessType==='game_zones';
const isDigitalStay=(store)=>store?.bookingMode==='digital_stay';
const isServiceStore=(store)=>store?.businessType==='services'||store?.businessType==='game_zones';
const bookingCopy=(store)=>isDigitalStay(store)
  ?{items:'Room Types',item:'Room Type',itemLower:'room type',resources:'Rooms',resource:'Room',resourceLower:'room',bookTitle:'Find Your Stay',bookButton:'+ Book a Stay',empty:'No stay bookings yet. Choose dates and a room type to get started.',historyItem:'Room Type',historyResource:'Room',after:'at check-in',completed:'after checkout'}
  :isGameZone(store)
...
function updateOrderAlertSound(){
  [...ringingIds].forEach(id=>{
    const stillRinging=state.orders.some(row=>row.id===id&&['Requested','Minimum Approval Requested'].includes(row.status))
      ...
      ||(state.bookings||[]).some(row=>row.id===id&&['Requested','Pending Payment','Identity Review'].includes(row.status));
...
async function refreshOwnerBookingsRealtime(){
  ...
  if(!bookings)return;
  let isNew=false;
  bookings.forEach(booking=>{if(['Requested','Pending Payment','Identity Review'].includes(booking.status)&&!knownBookingIds.has(booking.id)){ringingIds.add(booking.id);isNew=true}knownBookingIds.add(booking.id)});
...
function renderPlanGate(){
  ...
  let statusNote='';
  if(paused)statusNote=`<div class="card"><p class="muted">Your G58 business subscription is currently paused by the G58 team. Contact G58 support to resume access.</p></div>`;
  ...
  else if(pendingTrial)statusNote=`<div class="card trial-request-status"><span class="chip due">Approval pending</span><h3>Free-trial request sent</h3><p class="muted">The G58 admin will review and activate your 30-day business-tools trial. This page checks the approval automatically; you may also sign in again later.</p><button class="btn secondary full" id="refreshTrialStatus" type="button">Refresh Approval Status</button></div>`;
  else if(approvedTrial)statusNote=`<div class="card trial-request-status"><span class="chip delivered">Approved</span><h3>Finishing your activation</h3><p class="muted">Your trial was approved. Refresh the status to enter your G58 business dashboard.</p><button class="btn secondary full" id="refreshTrialStatus" type="button">Refresh Approval Status</button></div>`;
  ...
  const planCards=DIGIT58_PLAN_PERIODS.map(period=>{
    ...
    return `<article class="plan-card">
      <span class="plan-badge">G58 BUSINESS MONTHLY</span>
      <h3>Monthly Subscription</h3>
      <div class="plan-price">${money(amount)}<small> / month</small></div>
      <p class="plan-note">Use up to five stores, service businesses, game zones or Digital Stay hotels. ₹699 is auto-debited monthly after mandate authorisation.</p>
...
async function loadOwnerData(){
  const ownerId=cloudOwnerId();if(!ownerId)return;
  const [stores,customers,cards,orders,promotions,cardPurchases,incomingBrandRequests,catalog,services,experts,bookings,stayExtras]=await Promise.all([
    ...
    api.list(stayExtraKind(ownerId)).catch(()=>[]),
  ]);
  ...
  state.catalog=catalog;state.services=services;state.experts=experts;state.bookings=bookings;state.stayExtras=stayExtras;
...
function siteFooter(forCustomer){
  const badge=forCustomer
    ...
    : `<a class="g58-app-badge" href="/downloads/GRAVITY58-Android-v2.0.apk" download aria-label="Download the Gravity58 Android app"><span class="g58-app-badge-icon">▶</span><span class="g58-app-badge-text"><small>Download Android</small><strong>Get G58 App</strong></span></a>`;
...
function subscriptionView(){
  ...
  const planLabel=onFreeTrial?'Free Trial':plan?'Monthly Subscription':'G58 Business Access';
  ...
  $('#page').innerHTML=`<div class="section-head"><div><h1>Subscription</h1><p class="muted">One simple plan for Refills stores, service bookings, game zones and Digital Stay hotels.</p></div></div>
    <div class="card" style="max-width:460px">
      ...
      ${onFreeTrial?`<p class="muted">After your free trial, keep up to five business locations live with the ₹699 monthly subscription below.</p>`:''}
    ...
    ${showPlans?`<div class="section-head"><h2>${onFreeTrial?'Continue with monthly access':'G58 Monthly Subscription'}</h2></div><div class="plan-grid">${DIGIT58_PLAN_PERIODS.map(period=>{
      const amount=digit58PlanAmount(digit58Pricing.monthly,period);
      return `<article class="plan-card">
        <span class="plan-badge">G58 BUSINESS MONTHLY</span>
        <h3>Monthly Subscription</h3>
        <div class="plan-price">${money(amount)}<small> / month</small></div>
        <p class="plan-note">Includes up to five stores, service businesses, game zones or hotels. ₹699 is auto-debited monthly through Razorpay after mandate authorisation. Cancel anytime — the current paid month is non-refundable.</p>
        <button class="btn full" data-subscribe-plan="${period.id}" type="button">Subscribe</button>
      </article>`;
    }).join('')}</div>`:''}`;
...
function openStoreForm(storeId=''){
  ...
  modal(storeId?'Edit Location':'Create Location',`<form id="storeForm"><div class="field"><label>Location name</label><input name="name" value="${html(store.name||'')}" required></div><div class="field"><label>Business type</label><div class="business-type-toggle"><label class="option-toggle"><input type="radio" name="businessType" value="products" ${(!store.businessType||store.businessType==='products')?'checked':''}><span><strong>Store</strong><small>Sell products — customers place orders you price and fulfil.</small></span></label><label class="option-toggle"><input type="radio" name="businessType" value="services" ${store.businessType==='services'&&!isGameZone(store)&&!isDigitalStay(store)?'checked':''}><span><strong>Service</strong><small>Offer bookable services — customers book a slot and pay a prepayment.</small></span></label><label class="option-toggle"><input type="radio" name="businessType" value="game_zones" ${isGameZone(store)?'checked':''}><span><strong>Game Zone</strong><small>Book box-cricket turfs, courts, tables, play stations or indoor games in timed slots.</small></span></label><label class="option-toggle"><input type="radio" name="businessType" value="digital_stay" ${isDigitalStay(store)?'checked':''}><span><strong>Digital Stay</strong><small>Run hotels online with room inventory, ID review, payments, secure room PINs and room service.</small></span></label></div></div><div id="minimumOrderSection" class="${isServiceStore(store)?'hidden':''}"><label class="option-toggle"><input id="minimumOrderEnabled" name="minimumOrderEnabled" type="checkbox" ${minimumEnabled?'checked':''}><span><strong>Enable minimum new order criteria</strong><small>Switch this off anytime. Refills and history Reorders never use this limit.</small></span></label><div class="field ${minimumEnabled?'':'hidden'}" id="minimumOrderValueField"><label>Minimum new order value (₹)</label><input name="minimumOrderValue" type="number" min="1" step="1" value="${configuredStoreMinimum(store)||''}" placeholder="Example: 500"><small class="muted">A customer below this value can request a one-order approval from you.</small></div></div><div class="form-grid"><div class="field"><label>Category</label><input name="category" value="${html(store.category||'')}" placeholder="Example: Boutique hotel"></div><div class="field"><label>City</label><input name="city" value="${html(store.city||'')}"></div></div><div class="field"><label>Customer highlight text <small>(optional)</small></label><input name="highlightText" maxlength="40" value="${html(store.highlightText||'')}" placeholder="Example: Breakfast Included"><small class="muted">Shown as bold orange text on this location's customer page.</small></div><div class="field"><label>Phone</label><input name="phone" value="${html(store.phone||'')}"></div><div class="field"><label>UPI ID <small>(for payment QR codes)</small></label><input name="upiId" value="${html(store.upiId||'')}" placeholder="yourbusiness@upi"></div><label class="option-toggle"><input id="razorpayEnabled" name="razorpayEnabled" type="checkbox" ${store.razorpayEnabled?'checked':''}><span><strong>Enable Razorpay payment link</strong><small>Optional — customers can open your Razorpay page after you set the amount.</small></span></label><div class="field ${store.razorpayEnabled?'':'hidden'}" id="razorpayLinkField"><label>Razorpay payment link</label><input name="razorpayLink" type="text" inputmode="url" value="${html(store.razorpayLink||'')}" placeholder="razorpay.me/@yourbusiness"><small class="muted">Only razorpay.me or rzp.io secure links are accepted. https:// is added automatically.</small></div><div class="field"><label>Description</label><textarea name="description">${html(store.description||'')}</textarea></div><button class="btn full">${storeId?'Save Location':'Create Location'}</button></form>`,()=>{
    ...
    $$('input[name="businessType"]').forEach(input=>input.onchange=()=>$('#minimumOrderSection').classList.toggle('hidden',input.value!=='products'));
    ...
    $('#storeForm').onsubmit=async event=>{
      ...
      const minimumOrderEnabled=raw.businessType==='products'&&minimumToggle.checked,minimumOrderValue=minimumOrderEnabled?Math.max(0,Number(raw.minimumOrderValue)||0):0;
      if(minimumOrderEnabled&&!minimumOrderValue)return toast('Enter the minimum new order value or switch the criteria off');
      const selectedBusinessType=['services','game_zones','digital_stay'].includes(raw.businessType)?raw.businessType:'products';
      const businessType=selectedBusinessType==='products'?'products':'services';
      const bookingMode=selectedBusinessType==='game_zones'?'game_zones':selectedBusinessType==='digital_stay'?'digital_stay':selectedBusinessType==='services'?'services':'';
      const defaultCategory=selectedBusinessType==='game_zones'?'Indoor Game Zone':selectedBusinessType==='digital_stay'?'Hotel & Stay':'General store';
      const values={name:raw.name.trim(),businessType,bookingMode,minimumOrderEnabled,minimumOrderValue,category:raw.category.trim()||defaultCategory,city:raw.city.trim(),highlightText:raw.highlightText.trim(),phone:raw.phone.trim(),upiId:raw.upiId.trim(),razorpayEnabled,razorpayLink:razorpayEnabled?razorpayLink:'',description:raw.description.trim()};
      button.disabled=true;
      try{
        if(storeId){
...
async function loadAndRenderCustomerView(store,customer){
  ...
  const [cards,orders,promotions,courses,brandRequestsForStore,services,experts,bookings,stayExtras]=await Promise.all([
    ...
    isDigitalStay(store)?api.list(stayExtraKind(store.ownerId)).catch(()=>[]):Promise.resolve([]),
  ]);
  ...
  customerRenderPending=false;
  renderCustomerCards(store,customer,myCards,myOrders,myPromotions,myCourses,myServices,myExperts,myBookings,stayExtras.filter(row=>row.storeId===store.id&&row.active!==false));
}
...
function stayGuestCard(booking,store){
  const pendingPayment=booking.status==='Pending Payment',confirmed=booking.status==='Confirmed';
  const canRoomService=confirmed&&indiaDateValue()>=booking.checkInDate&&indiaDateValue()<=booking.checkOutDate;
  return `<article class="card stay-guest-card"><div class="section-head"><div><span class="stay-kicker">${html(booking.checkInDate)} → ${html(booking.checkOutDate)}</span><h3>${html(booking.serviceName)}</h3></div><span class="chip ${confirmed?'delivered':'due'}">${html(booking.status)}</span></div><div class="stay-booking-facts"><span><small>Room</small><strong>${html(booking.expertName||'After ID approval')}</strong></span><span><small>Guests</small><strong>${Number(booking.guests)||1}</strong></span><span><small>Nights</small><strong>${Number(booking.nights)||1}</strong></span><span><small>Total</small><strong>${money(booking.price)}</strong></span></div>${booking.identityStatus==='Pending Review'?'<div class="stay-privacy-note"><strong>ID safely submitted</strong><p>The hotel will verify it, allocate an available room and permanently delete the image.</p></div>':''}${booking.identityStatus==='Rejected'?`<div class="stay-privacy-note error"><strong>ID could not be verified</strong><p>${html(booking.identityRejectedReason||'Contact the hotel for help.')}</p></div>`:''}${pendingPayment?`<div class="stay-payment"><h4>Pay ${money(booking.price)}</h4>${booking.upiUri&&!booking.paymentMarkedAt?`<div class="qr" id="stay-qr-${html(booking.id)}"></div><a class="btn small secondary" href="${html(booking.upiUri)}">Open UPI App</a>`:''}${store.razorpayEnabled&&store.razorpayLink?`<a class="btn small secondary" href="${html(normaliseRazorpayLink(store.razorpayLink))}" target="_blank" rel="noopener">Pay with Razorpay</a>`:''}${booking.paymentMarkedAt?'<p class="chip due">Payment submitted for hotel verification</p>':'<button class="btn small green" data-mark-stay-paid="'+html(booking.id)+'">I Have Paid</button>'}</div>`:''}${confirmed?`<div class="stay-confirmed"><strong>✓ Room confirmed</strong><p>Check-in ${html(store.checkInTime||booking.startTime||'14:00')} · Check-out ${html(store.checkOutTime||'11:00')}</p>${booking.digitalLockPin?`<div class="stay-pin"><small>PRIVATE ROOM PIN</small><strong>${html(booking.digitalLockPin)}</strong><p>Show only to authorised guests.</p></div>`:'<p class="muted">Collect the room key or access instructions at reception.</p>'}${canRoomService?`<button class="btn full" data-room-service="${html(booking.id)}">🛎 Order Food or Room Service</button>`:''}</div>`:''}${bookingChatMarkup(booking,'customer')}</article>`;
}
function renderDigitalStayCustomer(store,customer,orders,promotions,roomTypes,bookings,stayExtras){
  ringCustomerPortalUpdates(store,customer,orders,bookings);
  const activeStays=bookings.filter(row=>row.bookingType==='digital_stay'&&!['Completed','Cancelled'].includes(row.status)).sort((a,b)=>new Date(a.checkInDate)-new Date(b.checkInDate));
  const history=bookings.filter(row=>row.bookingType==='digital_stay'&&['Completed','Cancelled'].includes(row.status)).sort((a,b)=>new Date(b.updatedAt||b.createdAt)-new Date(a.updatedAt||a.createdAt));
  const roomOrders=orders.filter(row=>row.orderType==='room_service');
  app.innerHTML=`<main class="public-store digital-stay-public">${customerBrandStrip()}${customerStoreHub(store)}<section class="stay-public-hero"><div><span class="stay-kicker">DIGITAL STAY · ${html(store.city||'BOOK DIRECT')}</span><h1>${html(store.name)}</h1><p>${html(store.description||'Book your room directly with the hotel—simple, private and secure.')}</p><div class="chips"><span class="chip">Best direct rate</span><span class="chip">Secure ID review</span><span class="chip">Live room status</span></div></div><div class="stay-hero-action"><small>ROOMS FROM</small><strong>${roomTypes.length?money(Math.min(...roomTypes.map(row=>Number(row.price)||0))):'Add rooms'}</strong><button class="btn full" id="bookStayBtn" ${store.emergencyMode||!roomTypes.length?'disabled':''}>${store.emergencyMode?'Bookings Paused':'Check Rooms & Book'}</button></div></section>${promotions.length?`<section class="promotion-strip"><div class="promotion-strip-head"><span>Hotel Offers</span></div><div class="promotion-rail"><div class="promotion-track"><div class="promotion-sequence">${promotions.map(row=>customerPromotionTicket(row)).join('')}</div></div></div></section>`:''}<section class="stay-room-types"><div class="section-head"><div><h2>Choose Your Room</h2><p class="muted">The final available room is allocated after private ID verification.</p></div></div><div class="grid catalog-grid">${roomTypes.map(row=>`<article class="card stay-room-public"><span class="chip">Sleeps ${Number(row.maxGuests)||2}</span><h3>${html(row.name)}</h3><p class="muted">${html(row.bedType||'Comfortable room')}</p>${row.description?`<p>${html(row.description)}</p>`:''}<strong>${money(row.price)} <small>/ night</small></strong><button class="btn small" data-book-room-type="${html(row.id)}">Check Availability</button></article>`).join('')||'<div class="empty">Room details will be available soon.</div>'}</div></section><section><div class="section-head"><h2>Your Stays</h2></div><div class="grid card-grid">${activeStays.map(row=>stayGuestCard(row,store)).join('')||'<div class="empty">No active stays. Choose your dates to begin.</div>'}</div></section>${roomOrders.length?`<section><div class="section-head"><h2>Room Service</h2></div><div class="grid card-grid">${roomOrders.map(row=>customerOrderMarkup(row,store)).join('')}</div></section>`:''}${history.length?`<section><div class="section-head"><h2>Past Stays</h2></div><div class="card table-wrap"><table><thead><tr><th>Room Type</th><th>Dates</th><th>Total</th><th>Status</th></tr></thead><tbody>${history.map(row=>`<tr><td>${html(row.serviceName)}</td><td>${html(row.checkInDate)} → ${html(row.checkOutDate)}</td><td>${money(row.price)}</td><td>${html(row.status)}</td></tr>`).join('')}</tbody></table></div></section>`:''}<div class="actions" style="margin-top:20px"><button class="btn secondary" id="viewAgreementBtn">Legal Agreement</button><button class="btn secondary" id="custLogout">Sign out</button></div></main>${siteFooter(true)}`;
  activeStays.filter(row=>row.status==='Pending Payment'&&row.upiUri&&!row.paymentMarkedAt).forEach(row=>{const target=document.getElementById(`stay-qr-${row.id}`);if(target&&window.QRCode)new QRCode(target,{text:row.upiUri,width:170,height:170})});
  bindCustomerStoreHub(store);$('#bookStayBtn')?.addEventListener('click',()=>openStayBookingModal(store,customer,roomTypes,stayExtras));$$('[data-book-room-type]').forEach(button=>button.onclick=()=>openStayBookingModal(store,customer,roomTypes,stayExtras,button.dataset.bookRoomType));$$('[data-mark-stay-paid]').forEach(button=>button.onclick=()=>markStayPaid(store,customer,button.dataset.markStayPaid));$$('[data-room-service]').forEach(button=>button.onclick=()=>openStayRoomServiceModal(store,customer,activeStays.find(row=>row.id===button.dataset.roomService),stayExtras));bindBookingChatForms(activeStays,'customer',()=>loadAndRenderCustomerView(store,customer));bindOrderChatForms(roomOrders,'customer',()=>loadAndRenderCustomerView(store,customer));$('#viewAgreementBtn').onclick=()=>openCustomerAgreementModal(store,customer);$('#custLogout').onclick=async()=>{stopCustomerRealtime();customerStoreLinks=[];await api.logout();location.hash=`store&owner=${encodeURIComponent(store.ownerId)}&store=${encodeURIComponent(store.id)}`;boot()};(typeof bindAndroidAppFooter==='function'&&bindAndroidAppFooter());
}
function openStayBookingModal(store,customer,roomTypes,stayExtras,selectedRoomTypeId=''){
  const addons=stayExtras.filter(row=>row.extraType==='addon'),today=indiaDateValue(),tomorrow=indiaDateValue(new Date(Date.now()+86400000));
  modal('Book Your Stay',`<form id="stayBookingForm"><div class="form-grid"><div class="field"><label>Check-in</label><input name="checkInDate" type="date" min="${today}" value="${today}" required></div><div class="field"><label>Check-out</label><input name="checkOutDate" type="date" min="${tomorrow}" value="${tomorrow}" required></div></div><div class="field"><label>Room type</label><select name="roomTypeId" required><option value="">Choose room type</option>${roomTypes.map(row=>`<option value="${html(row.id)}" ${selectedRoomTypeId===row.id?'selected':''}>${html(row.name)} · ${money(row.price)}/night</option>`).join('')}</select></div><div id="stayAvailability" class="stay-availability">Choose dates and a room type to check live availability.</div><div class="form-grid"><div class="field"><label>Guest name</label><input name="customerName" value="${html(customer.customerName||session?.name||'')}" required></div><div class="field"><label>Mobile number</label><input name="phone" type="tel" value="${html(customer.phone||'')}" required></div></div><div class="field"><label>Number of guests</label><input name="guests" type="number" min="1" max="20" value="1" required></div>${addons.length?`<fieldset class="stay-addon-list"><legend>Optional Add-ons</legend>${addons.map(row=>`<label><input type="checkbox" data-stay-addon="${html(row.id)}"><span><strong>${html(row.name)}</strong><small>${money(row.price)}</small></span><input type="number" min="1" max="10" value="1" data-stay-addon-qty="${html(row.id)}" aria-label="${html(row.name)} quantity"></label>`).join('')}</fieldset>`:''}<div class="field"><label>Government ID image</label><input name="identity" type="file" accept="image/jpeg,image/png,image/webp" required><small class="muted">JPG, PNG or WebP up to 5 MB. Only the hotel can review it; approval or rejection permanently deletes the image.</small></div><div id="stayBookingTotal" class="stay-total"></div><button class="btn full" type="submit">Submit ID & Request Room</button></form>`,()=>{
    const form=$('#stayBookingForm'),availability=$('#stayAvailability'),total=$('#stayBookingTotal');
    const refreshTotal=()=>{
      const type=roomTypes.find(row=>row.id===form.roomTypeId.value);
      const nights=Math.max(1,Math.round((new Date(form.checkOutDate.value)-new Date(form.checkInDate.value))/86400000)||1);
      const addonTotal=$$('[data-stay-addon]:checked').reduce((sum,input)=>{
        const row=addons.find(item=>item.id===input.dataset.stayAddon);
        const quantityInput=$(`[data-stay-addon-qty="${input.dataset.stayAddon}"]`);
        return sum+(Number(row?.price)||0)*(Number(quantityInput?.value)||1);
      },0);
      total.textContent=type?`Estimated total: ${money((Number(type.price)||0)*nights+addonTotal)} for ${nights} night${nights===1?'':'s'}`:'';
    };
    const check=async()=>{const roomTypeId=form.roomTypeId.value,checkInDate=form.checkInDate.value,checkOutDate=form.checkOutDate.value;form.checkOutDate.min=indiaDateValue(new Date(new Date(`${checkInDate}T00:00:00`).getTime()+86400000));refreshTotal();if(!roomTypeId||!checkInDate||!checkOutDate||checkOutDate<=checkInDate){availability.textContent='Choose a valid room type and date range.';return}availability.textContent='Checking live availability…';try{const result=await api.executeFunction(api.config.digitalOrderFunctionId,{action:'digit58-get-stay-availability',ownerId:store.ownerId,storeId:store.id,roomTypeId,checkInDate,checkOutDate});availability.innerHTML=result.available?`<strong>${result.available} room${result.available===1?'':'s'} available</strong>`:'<strong>No rooms available for these dates.</strong>'}catch(error){availability.textContent=error.message||'Could not check availability'}};
    form.onchange=event=>{if(['roomTypeId','checkInDate','checkOutDate'].includes(event.target.name))check();else refreshTotal()};check();
    form.onsubmit=async event=>{event.preventDefault();const button=event.submitter,values=Object.fromEntries(new FormData(form)),file=form.identity.files[0];if(values.checkOutDate<=values.checkInDate)return toast('Check-out must be after check-in');button.disabled=true;button.textContent='Uploading private ID…';let uploaded;try{uploaded=await api.uploadStayIdentity(file);button.textContent='Creating stay request…';const selectedAddons=$$('[data-stay-addon]:checked').map(input=>({id:input.dataset.stayAddon,qty:Number($(`[data-stay-addon-qty="${input.dataset.stayAddon}"]`).value)||1}));await api.executeFunction(api.config.digitalOrderFunctionId,{action:'digit58-create-stay-booking',ownerId:store.ownerId,storeId:store.id,roomTypeId:values.roomTypeId,checkInDate:values.checkInDate,checkOutDate:values.checkOutDate,guests:Number(values.guests),customerName:values.customerName,customerEmail:customer.customerEmail||session?.email||'',phone:values.phone,addons:selectedAddons,identityFileId:uploaded.fileId,identityUrl:uploaded.mediaUrl,identityName:uploaded.mediaName,identityType:uploaded.mediaType});closeModal();await loadAndRenderCustomerView(store,customer);toast('Stay requested. The hotel will verify your ID and allocate a room.')}catch(error){if(uploaded?.fileId)await api.removeAdMedia(uploaded.fileId).catch(()=>{});button.disabled=false;button.textContent='Submit ID & Request Room';toast(error.message||'Could not request this stay')}};
  });
}
async function markStayPaid(store,customer,bookingId){const booking=customerBookingsCache.find(row=>row.id===bookingId);if(!booking||!confirm(`Confirm that you paid ${money(booking.price)}? The hotel will verify it.`))return;try{await api.executeFunction(api.config.digitalOrderFunctionId,{action:'digit58-mark-stay-payment',ownerId:store.ownerId,bookingId});await loadAndRenderCustomerView(store,customer);toast('Payment submitted for hotel verification')}catch(error){toast(error.message||'Could not submit payment')}}
function openStayRoomServiceModal(store,customer,booking,stayExtras){if(!booking)return;const items=stayExtras.filter(row=>['food','service'].includes(row.extraType));modal('Room Service',`<form id="stayRoomServiceForm"><p class="muted">Deliver to <strong>Room ${html(booking.expertName)}</strong></p><div class="stay-service-menu">${items.map(row=>`<label><input type="checkbox" data-stay-service="${html(row.id)}"><span><strong>${html(row.name)}</strong><small>${html(row.extraType==='food'?'Food':'Service')} · ${money(row.price)}</small></span><input type="number" min="1" max="20" value="1" data-stay-service-qty="${html(row.id)}"></label>`).join('')||'<div class="empty">Room-service items are not configured yet.</div>'}</div><button class="btn full" type="submit" ${items.length?'':'disabled'}>Send Request</button></form>`,()=>{$('#stayRoomServiceForm').onsubmit=async event=>{event.preventDefault();const selected=$$('[data-stay-service]:checked').map(input=>({id:input.dataset.stayService,qty:Number($(`[data-stay-service-qty="${input.dataset.stayService}"]`).value)||1}));if(!selected.length)return toast('Choose at least one item');const button=event.submitter;button.disabled=true;try{await api.executeFunction(api.config.digitalOrderFunctionId,{action:'digit58-create-room-service-order',ownerId:store.ownerId,bookingId:booking.id,items:selected});closeModal();await loadAndRenderCustomerView(store,customer);toast('Room-service request sent')}catch(error){button.disabled=false;toast(error.message||'Could not send room-service request')}}});}
function renderCustomerCards(store,customer,cards,orders=[],promotions=[],courses=[],services=[],experts=[],bookings=[],stayExtras=[]){
  if(isDigitalStay(store))return renderDigitalStayCustomer(store,customer,orders,promotions,services,bookings,stayExtras);
...
function stayRoomTypesView(store){
  refreshView=servicesView;
  const rows=(state.services||[]).filter(row=>row.storeId===store.id);
  $('#page').innerHTML=`<div class="section-head"><div><span class="stay-kicker">DIGITAL STAY</span><h1>Room Types</h1><p class="muted">Create the choices guests see, such as Deluxe, Suite or Family Room.</p></div><button class="btn" id="addStayRoomType">+ Add Room Type</button></div><div class="grid catalog-grid">${rows.map(row=>`<article class="card stay-room-type-card"><div><span class="chip ${row.active===false?'due':'delivered'}">${row.active===false?'Hidden':'Bookable'}</span><h3>${html(row.name)}</h3><p class="muted">${html(row.bedType||'Standard bed')} · Up to ${Number(row.maxGuests)||2} guests</p><strong>${money(row.price)} / night</strong>${row.description?`<p class="muted">${html(row.description)}</p>`:''}</div><div class="actions"><button class="btn small" data-edit-service="${html(row.id)}">Edit</button><button class="btn small red" data-remove-service="${html(row.id)}">Remove</button></div></article>`).join('')||'<div class="empty">Add a room type, then add its physical rooms.</div>'}</div>`;
  $('#addStayRoomType').onclick=()=>openStayRoomTypeForm(store);
  $$('[data-edit-service]').forEach(button=>button.onclick=()=>openStayRoomTypeForm(store,button.dataset.editService));
  $$('[data-remove-service]').forEach(button=>button.onclick=()=>removeService(button.dataset.removeService));
}
function openStayRoomTypeForm(store,serviceId=''){
  const row=(state.services||[]).find(item=>item.id===serviceId)||{};
  modal(serviceId?'Edit Room Type':'Add Room Type',`<form id="stayRoomTypeForm"><div class="field"><label>Room type name</label><input name="name" value="${html(row.name||'')}" placeholder="Example: Deluxe King Room" required></div><div class="form-grid"><div class="field"><label>Nightly price (₹)</label><input name="price" type="number" min="0" step="0.01" value="${row.price??''}" required></div><div class="field"><label>Maximum guests</label><input name="maxGuests" type="number" min="1" max="20" value="${row.maxGuests||2}" required></div></div><div class="field"><label>Bed type</label><input name="bedType" value="${html(row.bedType||'')}" placeholder="King, Queen, Twin or Dormitory"></div><div class="field"><label>Description</label><textarea name="description" placeholder="Room highlights and included facilities">${html(row.description||'')}</textarea></div><label class="option-toggle"><input name="active" type="checkbox" ${row.active===false?'':'checked'}><span><strong>Available to book</strong><small>Turn this off to hide the room type without deleting past bookings.</small></span></label><button class="btn full" type="submit">${serviceId?'Save Room Type':'Add Room Type'}</button></form>`,()=>{
    $('#stayRoomTypeForm').onsubmit=async event=>{
      event.preventDefault();const values=Object.fromEntries(new FormData(event.target)),button=event.submitter;
      const changes={name:(values.name||'').trim(),price:Math.max(0,Number(values.price)||0),maxGuests:Math.max(1,Number(values.maxGuests)||2),bedType:(values.bedType||'').trim(),description:(values.description||'').trim(),active:values.active==='on'};
      if(!changes.name)return toast('Enter the room type name');button.disabled=true;
      try{if(serviceId){await api.update(serviceKind(store.ownerId),serviceId,changes);Object.assign(row,changes)}else{const record={id:id('roomtype'),ownerId:store.ownerId,storeId:store.id,...changes,createdAt:now()};await api.create(serviceKind(store.ownerId),record,record.id,api.permissionSet?.(serviceKind(store.ownerId),store.ownerId,true));state.services.push(record)}save();closeModal();servicesView();toast('Room type saved')}catch(error){button.disabled=false;toast(error.message||'Could not save room type')}
    };
  });
}
function stayRoomsView(store){
  refreshView=expertsView;
  const rows=(state.experts||[]).filter(row=>row.storeId===store.id),types=(state.services||[]).filter(row=>row.storeId===store.id);
  $('#page').innerHTML=`<div class="section-head"><div><span class="stay-kicker">ROOM INVENTORY</span><h1>Rooms</h1><p class="muted">Add every physical room once. Availability is calculated automatically from confirmed and pending stays.</p></div><button class="btn" id="addStayRoom" ${types.length?'':'disabled'}>+ Add Room</button></div>${types.length?'':'<div class="empty">Create a Room Type before adding rooms.</div>'}<div class="grid catalog-grid">${rows.map(row=>{const type=types.find(item=>item.id===row.roomTypeId);return `<article class="card stay-room-card"><span class="chip ${row.active===false?'due':'delivered'}">${row.active===false?'Out of service':'Ready'}</span><h3>${html(row.name)}</h3><p class="muted">${html(type?.name||'Room type removed')}${row.floor?` · Floor ${html(row.floor)}`:''}</p>${row.digitalLockEnabled?'<p class="chip">🔐 Digital PIN enabled</p>':''}<div class="actions"><button class="btn small" data-edit-expert="${html(row.id)}">Edit</button><button class="btn small red" data-remove-expert="${html(row.id)}">Remove</button></div></article>`}).join('')||''}</div>`;
  $('#addStayRoom')?.addEventListener('click',()=>openStayRoomForm(store));
  $$('[data-edit-expert]').forEach(button=>button.onclick=()=>openStayRoomForm(store,button.dataset.editExpert));
  $$('[data-remove-expert]').forEach(button=>button.onclick=()=>removeExpert(button.dataset.removeExpert));
}
function openStayRoomForm(store,expertId=''){
  const row=(state.experts||[]).find(item=>item.id===expertId)||{},types=(state.services||[]).filter(item=>item.storeId===store.id);
  modal(expertId?'Edit Room':'Add Room',`<form id="stayRoomForm"><div class="field"><label>Room name or number</label><input name="name" value="${html(row.name||'')}" placeholder="Example: 204" required></div><div class="field"><label>Room type</label><select name="roomTypeId" required><option value="">Choose room type</option>${types.map(type=>`<option value="${html(type.id)}" ${row.roomTypeId===type.id?'selected':''}>${html(type.name)}</option>`).join('')}</select></div><div class="field"><label>Floor <small>(optional)</small></label><input name="floor" value="${html(row.floor||'')}" placeholder="Example: 2"></div><label class="option-toggle"><input name="digitalLockEnabled" type="checkbox" ${row.digitalLockEnabled?'checked':''}><span><strong>Digital door-lock PIN</strong><small>After payment is verified, G58 creates a private 6-digit PIN for this stay. Use only with a compatible lock or front-desk process.</small></span></label><label class="option-toggle"><input name="active" type="checkbox" ${row.active===false?'':'checked'}><span><strong>Room in service</strong><small>Turn off during maintenance to remove it from availability.</small></span></label><button class="btn full" type="submit">${expertId?'Save Room':'Add Room'}</button></form>`,()=>{
    $('#stayRoomForm').onsubmit=async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.target)),button=event.submitter;const type=types.find(item=>item.id===values.roomTypeId);if(!type)return toast('Choose a room type');const changes={name:(values.name||'').trim(),roomTypeId:type.id,roomTypeName:type.name,floor:(values.floor||'').trim(),digitalLockEnabled:values.digitalLockEnabled==='on',active:values.active==='on'};if(!changes.name)return toast('Enter the room name or number');button.disabled=true;try{if(expertId){await api.update(expertKind(store.ownerId),expertId,changes);Object.assign(row,changes)}else{const record={id:id('room'),ownerId:store.ownerId,storeId:store.id,...changes,createdAt:now()};await api.create(expertKind(store.ownerId),record,record.id,api.permissionSet?.(expertKind(store.ownerId),store.ownerId,true));state.experts.push(record)}save();closeModal();expertsView();toast('Room saved')}catch(error){button.disabled=false;toast(error.message||'Could not save room')}};
  });
}
function staySettingsView(store){
  refreshView=availabilityView;
  $('#page').innerHTML=`<div class="section-head"><div><span class="stay-kicker">GUEST ARRIVAL</span><h1>Stay Settings</h1><p class="muted">Set your arrival policy and pause new bookings instantly when needed.</p></div></div><div class="card" style="max-width:620px"><form id="staySettingsForm"><div class="form-grid"><div class="field"><label>Check-in time</label><input name="checkInTime" type="time" value="${html(store.checkInTime||'14:00')}" required></div><div class="field"><label>Check-out time</label><input name="checkOutTime" type="time" value="${html(store.checkOutTime||'11:00')}" required></div></div><div class="field"><label>How far ahead can guests book? (days)</label><input name="preBookingWindowDays" type="number" min="1" max="730" value="${store.preBookingWindowDays||365}" required></div><label class="option-toggle"><input name="emergencyMode" type="checkbox" ${store.emergencyMode?'checked':''}><span><strong>Pause new stay bookings</strong><small>Existing stays remain visible and active.</small></span></label><button class="btn full" type="submit">Save Stay Settings</button></form></div>`;
  $('#staySettingsForm').onsubmit=async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.target)),button=event.submitter,changes={checkInTime:values.checkInTime,checkOutTime:values.checkOutTime,preBookingWindowDays:Math.max(1,Number(values.preBookingWindowDays)||365),emergencyMode:values.emergencyMode==='on'};button.disabled=true;try{await api.update(storeKind(store.ownerId),store.id,changes);Object.assign(store,changes);save();toast('Stay settings saved')}catch(error){button.disabled=false;toast(error.message||'Could not save stay settings')}};
}
function stayExtrasView(){
  refreshView=stayExtrasView;const store=activeStore(),rows=(state.stayExtras||[]).filter(row=>row.storeId===store.id);
  $('#page').innerHTML=`<div class="section-head"><div><span class="stay-kicker">EXTRAS</span><h1>Add-ons & Room Service</h1><p class="muted">Add-ons are selected during booking. Food and services become available after check-in.</p></div><button class="btn" id="addStayExtra">+ Add Item</button></div><div class="grid catalog-grid">${rows.map(row=>`<article class="card"><span class="chip">${row.extraType==='addon'?'Booking add-on':row.extraType==='food'?'Food':'Room service'}</span><h3>${html(row.name)}</h3><strong>${money(row.price)}</strong>${row.description?`<p class="muted">${html(row.description)}</p>`:''}<div class="actions"><button class="btn small" data-edit-stay-extra="${html(row.id)}">Edit</button><button class="btn small red" data-remove-stay-extra="${html(row.id)}">Remove</button></div></article>`).join('')||'<div class="empty">No extras yet. Add breakfast, extra bed, meals or housekeeping.</div>'}</div>`;
  $('#addStayExtra').onclick=()=>openStayExtraForm(store);$$('[data-edit-stay-extra]').forEach(button=>button.onclick=()=>openStayExtraForm(store,button.dataset.editStayExtra));$$('[data-remove-stay-extra]').forEach(button=>button.onclick=()=>removeStayExtra(button.dataset.removeStayExtra));
}
function openStayExtraForm(store,extraId=''){
  const row=(state.stayExtras||[]).find(item=>item.id===extraId)||{};
  modal(extraId?'Edit Item':'Add Item',`<form id="stayExtraForm"><div class="field"><label>Item name</label><input name="name" value="${html(row.name||'')}" placeholder="Breakfast, Extra bed or Housekeeping" required></div><div class="form-grid"><div class="field"><label>Available as</label><select name="extraType"><option value="addon" ${row.extraType==='addon'?'selected':''}>Booking add-on</option><option value="food" ${row.extraType==='food'?'selected':''}>Food order</option><option value="service" ${row.extraType==='service'?'selected':''}>Room service</option></select></div><div class="field"><label>Price (₹)</label><input name="price" type="number" min="0" step="0.01" value="${row.price??''}" required></div></div><div class="field"><label>Description</label><textarea name="description">${html(row.description||'')}</textarea></div><label class="option-toggle"><input name="active" type="checkbox" ${row.active===false?'':'checked'}><span><strong>Available</strong><small>Guests can select this item.</small></span></label><button class="btn full" type="submit">Save Item</button></form>`,()=>{$('#stayExtraForm').onsubmit=async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.target)),button=event.submitter,changes={name:(values.name||'').trim(),extraType:['addon','food','service'].includes(values.extraType)?values.extraType:'addon',price:Math.max(0,Number(values.price)||0),description:(values.description||'').trim(),active:values.active==='on'};if(!changes.name)return toast('Enter an item name');button.disabled=true;try{if(extraId){await api.update(stayExtraKind(store.ownerId),extraId,changes);Object.assign(row,changes)}else{const record={id:id('stayextra'),ownerId:store.ownerId,storeId:store.id,...changes,createdAt:now()};await api.create(stayExtraKind(store.ownerId),record,record.id,api.permissionSet?.(stayExtraKind(store.ownerId),store.ownerId,true));state.stayExtras.push(record)}save();closeModal();stayExtrasView();toast('Item saved')}catch(error){button.disabled=false;toast(error.message||'Could not save item')}}});
}
async function removeStayExtra(extraId){if(!confirm('Remove this item? Past bookings will keep their original details.'))return;try{await api.remove(stayExtraKind(cloudOwnerId()),extraId);state.stayExtras=state.stayExtras.filter(row=>row.id!==extraId);stayExtrasView();toast('Item removed')}catch(error){toast(error.message||'Could not remove item')}}
function servicesView(){
  refreshView=servicesView;
  const store=activeStore();
  if(isDigitalStay(store))return stayRoomTypesView(store);
  const copy=bookingCopy(store),gameZone=isGameZone(store);
...
function openServiceForm(store,serviceId=''){
  if(isDigitalStay(store))return openStayRoomTypeForm(store,serviceId);
...
function availabilityView(){
  ...
  if(isDigitalStay(store))return staySettingsView(store);
  const gameZone=isGameZone(store);
...
function expertsView(){
  refreshView=expertsView;
  const store=activeStore();
  if(isDigitalStay(store))return stayRoomsView(store);
  const copy=bookingCopy(store),gameZone=isGameZone(store);
...
function openExpertForm(store,expertId=''){
  if(isDigitalStay(store))return openStayRoomForm(store,expertId);
...
function stayBookingOwnerCard(booking,store){
  const room=booking.expertName||'Not allocated',ringing=ringingIds.has(booking.id);
  const actions=booking.status==='Identity Review'
    ?`<button class="btn small green" data-review-stay="${html(booking.id)}">Review ID & Allocate</button><button class="btn small red" data-reject-stay="${html(booking.id)}">Reject</button>`
    :booking.status==='Pending Payment'
      ?`${booking.paymentMarkedAt?'<span class="chip due">Guest says paid</span>':''}<button class="btn small green" data-confirm-stay-payment="${html(booking.id)}">Payment Received</button><button class="btn small secondary" data-clear-stay-payment="${html(booking.id)}">Not Received</button>`
      :booking.status==='Confirmed'
        ?`<button class="btn small green" data-complete-stay="${html(booking.id)}">Check Out</button><button class="btn small red" data-cancel-stay="${html(booking.id)}">Cancel</button>`:'';
  return `<article class="card stay-booking-card ${ringing?'incoming-order':''}">${ringing?'<span class="incoming-order-beacon"></span>':''}<div class="section-head"><div><span class="stay-kicker">${html(booking.checkInDate)} → ${html(booking.checkOutDate)}</span><h3>${html(booking.customerName||'Guest')}</h3></div><span class="chip ${booking.status==='Confirmed'?'delivered':'due'}">${html(booking.status)}</span></div><div class="stay-booking-facts"><span><small>Room Type</small><strong>${html(booking.serviceName)}</strong></span><span><small>Room</small><strong>${html(room)}</strong></span><span><small>Guests</small><strong>${Number(booking.guests)||1}</strong></span><span><small>Total</small><strong>${money(booking.price)}</strong></span></div><p class="muted">${html(booking.phone||'')} · ${Number(booking.nights)||1} night${Number(booking.nights)===1?'':'s'}</p>${booking.addons?.length?`<p class="muted">Add-ons: ${booking.addons.map(item=>`${html(item.name)} × ${Number(item.qty)||1}`).join(', ')}</p>`:''}${booking.digitalLockPin?`<div class="stay-pin"><small>PRIVATE ROOM PIN</small><strong>${html(booking.digitalLockPin)}</strong></div>`:''}<div class="actions">${actions}</div>${bookingChatMarkup(booking,'owner')}</article>`;
}
function stayBookingsView(store){
  refreshView=bookingsView;
  const rows=(state.bookings||[]).filter(row=>row.storeId===store.id&&row.bookingType==='digital_stay'&&!['Completed','Cancelled'].includes(row.status)).sort((a,b)=>new Date(a.checkInDate)-new Date(b.checkInDate));
  $('#page').innerHTML=`<div class="section-head"><div><span class="stay-kicker">LIVE OPERATIONS</span><h1>Stay Bookings</h1><p class="muted">Verify identity, allocate an available room, confirm payment and share the private room PIN.</p></div><span class="chip">${rows.length} active</span></div><div class="stay-flow"><span>1 · ID review</span><span>2 · Room allocation</span><span>3 · Payment</span><span>4 · Check-in</span></div><div class="grid card-grid">${rows.map(row=>stayBookingOwnerCard(row,store)).join('')||'<div class="empty">No active stay bookings.</div>'}</div>`;
  $$('[data-review-stay]').forEach(button=>button.onclick=()=>openStayIdentityReview(store,button.dataset.reviewStay));
  $$('[data-reject-stay]').forEach(button=>button.onclick=()=>rejectStayIdentity(store,button.dataset.rejectStay));
  $$('[data-confirm-stay-payment]').forEach(button=>button.onclick=()=>confirmStayPayment(store,button.dataset.confirmStayPayment));
  $$('[data-clear-stay-payment]').forEach(button=>button.onclick=()=>clearStayPayment(button.dataset.clearStayPayment));
  $$('[data-complete-stay]').forEach(button=>button.onclick=()=>finishStay(button.dataset.completeStay,'Completed'));
  $$('[data-cancel-stay]').forEach(button=>button.onclick=()=>finishStay(button.dataset.cancelStay,'Cancelled'));
  bindBookingChatForms(rows,'owner',bookingsView);
}
function openStayIdentityReview(store,bookingId){
  const booking=(state.bookings||[]).find(row=>row.id===bookingId);if(!booking)return;
  const rooms=(state.experts||[]).filter(row=>row.storeId===store.id&&row.roomTypeId===booking.serviceId&&row.active!==false);
  modal('Verify Guest ID',`<div class="stay-privacy-note"><strong>Private document</strong><p>The ID is visible only for verification and will be permanently deleted when you approve or reject it.</p></div>${booking.identityUrl?`<a class="btn secondary full" href="${html(booking.identityUrl)}" target="_blank" rel="noopener">View uploaded ID</a>`:'<p class="muted">ID preview is unavailable.</p>'}<form id="stayIdentityForm" style="margin-top:16px"><div class="field"><label>Allocate room</label><select name="roomId" required><option value="">Choose an available room</option>${rooms.map(room=>`<option value="${html(room.id)}">${html(room.name)}${room.floor?` · Floor ${html(room.floor)}`:''}</option>`).join('')}</select><small class="muted">The server checks availability again before allocation.</small></div><button class="btn full green" type="submit">Approve ID & Send Payment</button></form>`,()=>{$('#stayIdentityForm').onsubmit=async event=>{event.preventDefault();const button=event.submitter,roomId=new FormData(event.target).get('roomId');if(!roomId)return toast('Choose a room');button.disabled=true;try{const result=await api.executeFunction(api.config.digitalOrderFunctionId,{action:'digit58-review-stay-identity',ownerId:store.ownerId,bookingId,roomId,approved:true});Object.assign(booking,result.booking||{});ringingIds.delete(bookingId);closeModal();bookingsView();toast('ID verified. Payment QR is now visible to the guest.')}catch(error){button.disabled=false;toast(error.message||'Could not approve this stay')}}});
}
async function rejectStayIdentity(store,bookingId){const booking=state.bookings.find(row=>row.id===bookingId);if(!booking)return;const reason=prompt('Reason for rejection','Identity document could not be verified.');if(reason===null)return;try{const result=await api.executeFunction(api.config.digitalOrderFunctionId,{action:'digit58-review-stay-identity',ownerId:store.ownerId,bookingId,approved:false,reason});Object.assign(booking,result.booking||{});ringingIds.delete(bookingId);bookingsView();toast('Stay request rejected and ID deleted')}catch(error){toast(error.message||'Could not reject stay')}}
async function confirmStayPayment(store,bookingId){const booking=state.bookings.find(row=>row.id===bookingId);if(!booking||!confirm(`Confirm ${money(booking.price)} was received?`))return;try{const result=await api.executeFunction(api.config.digitalOrderFunctionId,{action:'digit58-confirm-stay-payment',ownerId:store.ownerId,bookingId});Object.assign(booking,result.booking||{});ringingIds.delete(bookingId);bookingsView();toast(booking.digitalLockPin?'Payment verified and private PIN generated':'Payment verified and room confirmed')}catch(error){toast(error.message||'Could not confirm payment')}}
async function clearStayPayment(bookingId){const booking=state.bookings.find(row=>row.id===bookingId);if(!booking)return;try{await api.update(bookingKind(booking.ownerId),bookingId,{paymentMarkedAt:'',updatedAt:now()});booking.paymentMarkedAt='';bookingsView();toast('Guest can submit payment again')}catch(error){toast(error.message||'Could not reopen payment')}}
async function finishStay(bookingId,status){const booking=state.bookings.find(row=>row.id===bookingId);if(!booking||!confirm(`${status==='Completed'?'Check out':'Cancel'} this stay?`))return;const stamp=now(),changes={status,digitalLockPin:'',updatedAt:stamp,...(status==='Completed'?{completedAt:stamp}:{cancelledAt:stamp})};try{await api.update(bookingKind(booking.ownerId),bookingId,changes);Object.assign(booking,changes);bookingsView();toast(status==='Completed'?'Guest checked out and PIN cleared':'Stay cancelled and PIN cleared')}catch(error){toast(error.message||'Could not update stay')}}
function roomServiceView(){
  refreshView=roomServiceView;const store=activeStore(),rows=activeOrders(state.orders).filter(row=>row.storeId===store.id&&row.orderType==='room_service').sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
  $('#page').innerHTML=`<div class="section-head"><div><span class="stay-kicker">IN-ROOM REQUESTS</span><h1>Room Service Orders</h1><p class="muted">Food and service requests arrive here and follow the normal order workflow.</p></div></div><div class="grid card-grid">${rows.map(row=>ownerOrderMarkup(row,true,false)).join('')||'<div class="empty">No active room-service orders.</div>'}</div>`;bindOwnerOrderActions();bindOrderChatForms(rows,'owner',roomServiceView);bindDeliveryShareButtons(rows);
}
function bookingsView(){
  refreshView=bookingsView;
  const store=activeStore();if(!store)return;
  if(isDigitalStay(store))return stayBookingsView(store);
  const today=indiaDateValue();
