...
function siteFooter(){return `<footer class="g58-site-footer"><div class="g58-site-footer-badge"><a class="g58-app-badge" href="/downloads/GRAVITY58-Android-v1.3.apk" download aria-label="Download the Gravity58 Android app"><span class="g58-app-badge-icon">▶</span><span class="g58-app-badge-text"><small>Download Android</small><strong>Get G58 App</strong></span></a></div><p class="g58-site-footer-note">© ${new Date().getFullYear()} Gravity58 · Refills</p></footer>`}
...
function directoryRowId(ownerId){return safeId('dir_refills_',ownerId,40)}
async function syncDirectoryListing(store,ownerId){
  const rowId=directoryRowId(ownerId);
  if(store.listedInSearch){
    const data={source:'refills',ownerId,storeId:store.id,name:store.name||'',category:store.category||'General store',city:store.city||'',url:publicStoreLink(store),updatedAt:now()};
    const permissions=api.permissionSet?.('directory',ownerId);
    try{await api.create('directory',data,rowId,permissions)}
    catch(error){
      if(error?.code!==409&&!/already exists/i.test(error?.message||''))throw error;
      await api.update('directory',rowId,data,permissions);
    }
  }else{
    try{await api.remove('directory',rowId)}catch(error){/* not listed, nothing to remove */}
  }
}
function openStoreForm(storeId=''){
  ...
  modal(storeId?'Edit Store':'Create Store',`<form id="storeForm"><div class="field"><label>Store name</label><input name="name" value="${html(store.name||'')}" required></div><label class="option-toggle"><input id="minimumOrderEnabled" name="minimumOrderEnabled" type="checkbox" ${minimumEnabled?'checked':''}><span><strong>Enable minimum new order criteria</strong><small>Switch this off anytime. Refills and history Reorders never use this limit.</small></span></label><div class="field ${minimumEnabled?'':'hidden'}" id="minimumOrderValueField"><label>Minimum new order value (₹)</label><input name="minimumOrderValue" type="number" min="1" step="1" value="${configuredStoreMinimum(store)||''}" placeholder="Example: 500"><small class="muted">A customer below this value can request a one-order approval from you.</small></div><div class="form-grid"><div class="field"><label>Category</label><input name="category" value="${html(store.category||'')}" placeholder="Example: Pharmacy"></div><div class="field"><label>City</label><input name="city" value="${html(store.city||'')}"></div></div><div class="field"><label>Customer highlight text <small>(optional)</small></label><input name="highlightText" maxlength="40" value="${html(store.highlightText||'')}" placeholder="Example: 20% Off"><small class="muted">Shown as bold orange text on this store's customer page.</small></div><div class="field"><label>Phone</label><input name="phone" value="${html(store.phone||'')}"></div><div class="field"><label>UPI ID <small>(for order payment QR codes)</small></label><input name="upiId" value="${html(store.upiId||'')}" placeholder="yourstore@upi"></div><label class="option-toggle"><input id="razorpayEnabled" name="razorpayEnabled" type="checkbox" ${store.razorpayEnabled?'checked':''}><span><strong>Enable Razorpay payment link</strong><small>Optional — customers can open your Razorpay page after you set the order amount.</small></span></label><div class="field ${store.razorpayEnabled?'':'hidden'}" id="razorpayLinkField"><label>Razorpay payment link</label><input name="razorpayLink" type="text" inputmode="url" value="${html(store.razorpayLink||'')}" placeholder="razorpay.me/@yourstore"><small class="muted">Only razorpay.me or rzp.io secure links are accepted. https:// is added automatically.</small><div class="razorpay-link-note"><strong>How reusable Razorpay.me links work</strong><small>Razorpay.me has no return-URL setting. After paying, the customer returns to the G58 tab and taps Payment completed. Always verify the payment in Razorpay before accepting the order.</small></div></div><div class="field"><label>Description</label><textarea name="description">${html(store.description||'')}</textarea></div><label class="option-toggle"><input id="listedInSearch" name="listedInSearch" type="checkbox" ${store.listedInSearch?'checked':''}><span><strong>List my store in G58 Search</strong><small>Lets customers find and open your store from the G58 search page.</small></span></label><button class="btn full">${storeId?'Save Store':'Create Store'}</button></form>`,()=>{
    ...
    $('#storeForm').onsubmit=async event=>{
      ...
      const listedInSearch=$('#listedInSearch').checked;
      const values={name:raw.name.trim(),minimumOrderEnabled,minimumOrderValue,category:raw.category.trim()||'General store',city:raw.city.trim(),highlightText:raw.highlightText.trim(),phone:raw.phone.trim(),upiId:raw.upiId.trim(),razorpayEnabled,razorpayLink:razorpayEnabled?razorpayLink:'',description:raw.description.trim(),listedInSearch};
      button.disabled=true;
      try{
        if(storeId){
          ...
          await syncDirectoryListing(store,ownerId);
        }else{
          ...
          await syncDirectoryListing(record,ownerId);
        }
