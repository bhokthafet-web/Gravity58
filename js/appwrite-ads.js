(() => {
  ...
  function permissionSet(kind, userId, includeAdminTeam = false) {
    ...
    const permissions = [];
    const readAny = ["advertisements", "slots", "posts", "digital_menus", "digital_menu_pricing"].includes(kind) || String(kind).startsWith("digital_menu_") && !["digital_menu_entitlements", "digital_menu_requests"].includes(kind) || String(kind).startsWith("digit58_store_");
