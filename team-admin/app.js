...
function configurationRequired(){app.innerHTML=`<main class="screen auth"><section class="auth-card glass"><div class="brand"><svg class="brand-mark" viewBox="0 0 120 120" fill="none" stroke="#6d5ef0" stroke-width="8" aria-hidden="true"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg><div><h2>Gravity58 Team Admin</h2><p class="tagline">Private administration</p></div></div><div class="notice"><strong>Administration service unavailable</strong><p>The secure administration service is not available. Please contact the Gravity58 technical team.</p></div><a class="btn full" href="../">Return to G58</a></section></main>`}
function login(){app.innerHTML=`<main class="screen auth"><section class="auth-card glass"><div class="brand"><svg class="brand-mark" viewBox="0 0 120 120" fill="none" stroke="#6d5ef0" stroke-width="8" aria-hidden="true"><circle cx="60" cy="26" r="15"/><circle cx="28" cy="82" r="15"/><circle cx="92" cy="82" r="15"/></svg><div><h2>Gravity58 Team Admin</h2><p class="tagline">G58 team members only</p></div></div><form id="login"><div class="field"><label>Team email</label><input name="email" type="email" required></div><div class="field"><label>Password</label><input name="password" type="password" required></div><button class="btn full">Secure Login</button></form><p class="muted" style="text-align:center">Access is restricted to authorised Gravity58 team members.</p></section></main>`;$('#login').onsubmit=async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.target));try{await api.login(values.email,values.password);user=await api.currentUser();if(!await api.isTeamAdmin()){await api.logout();throw new Error('This account is not a member of the G58 administration team.')}await loadData();shell()}catch(error){toast(error.message||'Login failed')}}}
