...
function marketplace(){const type='business';$('#page').innerHTML=`<div class="section-head"><div><h1>Business Cards</h1><p class="muted">${data.businesses.length} digital business card(s). Edit records or select multiple entries for permanent deletion.</p></div><button class="btn red" id="deleteSelected">Delete selected</button></div><div class="card table-wrap" id="marketTable" style="margin-top:14px"></div>`;const draw=()=>{const rows=data.businesses;$('#marketTable').innerHTML=`<table><thead><tr><th></th><th>Title</th><th>Location</th><th>Account</th><th>Status</th><th>Action</th></tr></thead><tbody>${rows.map(row=>`<tr><td><input type="checkbox" data-post-id="${esc(row.id)}"></td><td>${esc(row.title)}</td><td>${esc(row.district||row.city||'')}</td><td>${esc(row.accountEmail||row.email||'')}</td><td>${row.blocked?'Blocked':'Active'}</td><td><button class="btn small" data-edit-post="${esc(row.id)}">Edit</button></td></tr>`).join('')||'<tr><td colspan="6">No business cards.</td></tr>'}</tbody></table>`;$$('[data-edit-post]').forEach(button=>button.onclick=()=>editPost(type,button.dataset.editPost))};draw();$('#deleteSelected').onclick=async()=>{const ids=$$('[data-post-id]:checked').map(input=>input.dataset.postId);if(!ids.length||!confirm(`Delete ${ids.length} selected record(s)?`))return;data.businesses=data.businesses.filter(row=>!ids.includes(row.id));await savePosts();marketplace()}}
