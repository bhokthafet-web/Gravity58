...
function overview(){
  ...
  $('#page').innerHTML=`<div class="section-head"><div><h1>Unified Administration</h1><p class="muted">Manage advertising, public posts, accounts and restaurant placements from one G58-only portal.</p></div><button class="btn" id="refresh">Refresh</button></div><div class="grid stats">${metric('Pending Bookings',data.bookings.filter(row=>['Requested','Proof Sent'].includes(row.status)).length)}${metric('Live Ads',data.advertisements.filter(row=>row.active&&(!row.expiresAt||new Date(row.expiresAt)>new Date())).length)}${metric('Business Cards',data.businesses.length)}${metric('Total Revenue',money(totalRevenue))}</div>
...
function marketplace(){
  let type='customer';
  $('#page').innerHTML=`<div class="section-head"><div><h1>Public Marketplace</h1><p class="muted">Review, edit or permanently delete customer requirements and digital business cards.</p></div><button class="btn red" id="deleteSelected">Delete selected</button></div><div class="admin-filter-bar"><button class="btn" data-market-type="customer">Customer Posts (${data.customers.length})</button><button class="btn secondary" data-market-type="business">Business Cards (${data.businesses.length})</button></div><div class="card table-wrap" id="marketTable" style="margin-top:14px"></div>`;
  const draw=()=>{
    const rows=type==='customer'?data.customers:data.businesses,label=type==='customer'?'customer posts':'business cards';
    $$('[data-market-type]').forEach(button=>button.className=`btn ${button.dataset.marketType===type?'':'secondary'}`);
    $('#marketTable').innerHTML=`<table><thead><tr><th></th><th>Title</th><th>Location</th><th>Account</th><th>Status</th><th>Action</th></tr></thead><tbody>${rows.map(row=>`<tr><td><input type="checkbox" data-post-id="${esc(row.id)}"></td><td>${esc(row.title)}</td><td>${esc(row.district||row.city||'')}</td><td>${esc(row.accountEmail||row.email||'')}</td><td>${row.blocked?'Blocked':'Active'}</td><td><button class="btn small" data-edit-post="${esc(row.id)}">Edit</button></td></tr>`).join('')||`<tr><td colspan="6">No ${label}.</td></tr>`}</tbody></table>`;
    $$('[data-edit-post]').forEach(button=>button.onclick=()=>editPost(type,button.dataset.editPost));
  };
  $$('[data-market-type]').forEach(button=>button.onclick=()=>{type=button.dataset.marketType;draw()});
  draw();
  $('#deleteSelected').onclick=async()=>{const ids=$$('[data-post-id]:checked').map(input=>input.dataset.postId);if(!ids.length||!confirm(`Delete ${ids.length} selected record(s)?`))return;const key=type==='customer'?'customers':'businesses';data[key]=data[key].filter(row=>!ids.includes(String(row.id)));await savePosts();marketplace()};
}
